...
export default function BetaGate(){
  ...
  const submitPrivacyRequest=async(type:PrivacyRequestRow["request_type"])=>{
    ...
    const {error}=await supabase.rpc("submit_privacy_request",{
      ...
      p_details:type==="revoke_coach_access"?"Please remove current Coach/team access for this Player.":"Submitted from the RC57 Privacy Center."
    });
    if(error){setPrivacyMessage(cleanConnectionError(error.message));return}
  ...
  return <div className="betaAppShell">
    <div className="betaRibbon">CLOSED BETA · RC57 · v72.3.107</div>
    ...
    {showLaunchChecklist&&!showDisclaimer&&<div className="betaModalOverlay"><div className="betaModalCard betaLaunchChecklist">
      <div className="sectionHead"><div><small>RC57 CLOSED BETA</small><h2>{access.role} Start Checklist</h2></div><button aria-label="Close checklist" onClick={()=>setShowLaunchChecklist(false)}>×</button></div>
      ...
      <div className="launchChecklistSteps">
        ...
        {access.role==="Admin"&&<><div><span>2</span><div><b>Approve invited emails</b><small>RC57 registration is email-approved for every role.</small></div></div><div><span>3</span><div><b>Watch privacy requests</b><small>Review export, deletion, and Coach-access requests in Beta Readiness.</small></div></div></>}
    ...
    {showAdmin&&access.role==="Admin"&&<div className="betaModalOverlay"><div className="betaAdminCard betaAdminInboxCard">
      <div className="sectionHead"><div><small>BETA ADMIN · RC57</small><h2>{adminSection==="readiness"?"Closed Beta Readiness":adminSection==="accounts"?"Account Access":adminSection==="family"?"Family & Account Diagnostics":"Beta Feedback Inbox"}</h2></div><button onClick={()=>setShowAdmin(false)}>×</button></div>
      ...
      {adminSection==="readiness"?<div className="betaReadinessPanel">
        ...
        <p className="coachGroupIntro">RC57 is email-approved. Approve the exact email for every Player, Parent, Coach, or Admin before they create an account.</p>
