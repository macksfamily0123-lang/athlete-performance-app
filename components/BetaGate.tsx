...
export default function BetaGate(){
  ...
  useEffect(()=>{
    ...
    navigator.serviceWorker.register("/sw.js?v=115",{updateViaCache:"none"}).then(registration=>registration.update()).catch(()=>{});
  },[]);
  ...
  const copyPlayerLoginAccess=async(athlete:AthleteRow)=>runConnectionAction("player-access",async()=>{
    ...
    const text=[
      `${athlete.display_name} already has an Elite Performance Player record managed by a Parent.`,
  ...
  const copyParentConnectionInvite=async()=>runConnectionAction("player-parent",async()=>{
    ...
    const code=String(data||"");
    const text=[
      `${selfAthlete.display_name} invited you to connect as a Parent in Elite Performance.`,
      `Open ${window.location.origin}`,
  ...
  const copyTeamInvite=async()=>{
    ...
    const text=[
      `You've been invited to join ${selectedTeam.name} in Elite Performance.`,
      `Open ${window.location.origin}`,
  ...
  const downloadPrivacyExport=async()=>{
    ...
    try{
      ...
      link.href=url;
      link.download=`elite-performance-privacy-export-${new Date().toISOString().slice(0,10)}.json`;
      link.click();
  ...
  const submitPrivacyRequest=async(type:PrivacyRequestRow["request_type"])=>{
    ...
    const {error}=await supabase.rpc("submit_privacy_request",{
      ...
      p_details:type==="revoke_coach_access"?"Please remove current Coach/team access for this Player.":"Submitted from the RC65 Privacy Center."
    });
    if(error){setPrivacyMessage(cleanConnectionError(error.message));return}
  ...
  if(signupComplete&&!user)return <div className="betaAuthShell"><div className="betaAuthCard signupConfirmationCard">
    ...
    <div className="signupConfirmationSteps">
      <div><span>1</span><p>Open the verification email from Elite Performance / Supabase.</p></div>
  ...
  if(passwordRecovery&&user)return <div className="betaAuthShell"><div className="betaAuthCard betaRecoveryCard">
    <div className="betaAuthBrand"><span><img src="/elite-performance-speed-e.svg" alt=""/></span><div><small>ACCOUNT RECOVERY</small><h1>Set a new password</h1><p>Choose a new password for your Elite Performance beta account.</p></div></div>
  ...
  if(!user)return <div className="betaAuthShell"><div className="betaAuthCard">
    <div className="betaAuthBrand"><span><img src="/elite-performance-speed-e.svg" alt=""/></span><div><small>ELITE PERFORMANCE · CLOSED BETA</small><h1>{authMode==="signin"?"Beta sign in":"Activate invited account"}</h1><p>Secure accounts with cloud-backed athlete data.</p></div></div>
  ...
  return <div className="betaAppShell">
    <div className="betaRibbon">CLOSED BETA · RC65 · v72.3.115</div>
    ...
    {showLaunchChecklist&&!showDisclaimer&&<div className="betaModalOverlay"><div className="betaModalCard betaLaunchChecklist">
      <div className="sectionHead"><div><small>RC65 CLOSED BETA</small><h2>{access.role} Start Checklist</h2></div><button aria-label="Close checklist" onClick={()=>setShowLaunchChecklist(false)}>×</button></div>
      ...
      <div className="launchChecklistSteps">
        ...
        {access.role==="Admin"&&<><div><span>2</span><div><b>Approve invited emails</b><small>RC65 registration is email-approved for every role.</small></div></div><div><span>3</span><div><b>Watch privacy requests</b><small>Review export, deletion, and Coach-access requests in Beta Readiness.</small></div></div></>}
    ...
    {showParentPlayers&&access.role==="Parent"&&<div className="betaModalOverlay"><div className="betaAdminCard parentFamilyCard connectionSetupCard">
      ...
      <div className="connectionRuleNotice">
        <span>1 ATHLETE</span>
        <div><b>Choose the correct way to add a Player</b><p>If the Player already has an Elite Performance account, use <strong>Connect Existing Player</strong>. Only use <strong>Create New Player</strong> when no Player record exists yet.</p></div>
      </div>
    ...
    {showAdmin&&access.role==="Admin"&&<div className="betaModalOverlay"><div className="betaAdminCard betaAdminInboxCard">
      <div className="sectionHead"><div><small>BETA ADMIN · RC65</small><h2>{adminSection==="readiness"?"Closed Beta Readiness":adminSection==="accounts"?"Account Access":adminSection==="family"?"Family & Account Diagnostics":"Beta Feedback Inbox"}</h2></div><button onClick={()=>setShowAdmin(false)}>×</button></div>
      ...
      {adminSection==="readiness"?<div className="betaReadinessPanel">
        ...
        <p className="coachGroupIntro">RC65 is email-approved. Approve the exact email for every Player, Parent, Coach, or Admin before they create an account.</p>
