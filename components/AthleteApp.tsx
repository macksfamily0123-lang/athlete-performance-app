...
type Profile={name:string;position:string;team:string;season:string;height:string;weight:string;handedness:"Right"|"Left";age?:string;sport?:Sport;photoUrl?:string};
...
type CompetitionLog={id:number;date:string;opponent:string;eventType:string;result:string;minutes:string;rating:number;notes:string;sport:Sport;stats:StatEntry[];location?:string;role?:string;keyWin?:string;improveNext?:string;confidence?:number;enteredByRole?:AccountRole;entryKind?:"Full"|"Score"};
type ReportNote={id:number;date:string;title:string;body:string};
type AthleteRecord={id:string;name:string;sport:Sport;position:string;team:string;season:string;height:string;weight:string;handedness:"Right"|"Left";age?:string;photoUrl?:string};
type RosterSummary={id:string;name:string;sport:Sport;position:string;team:string;goals:number;workouts:number;tests:number;competitions:number;readiness:number;score:number;photoUrl?:string};
type DailyLoad={date:string;label:string;load:number;workouts:number;events:number};
type QuickAction={id:string;label:string;tab:Tab;keywords:string[];target?:string};
...
type NavigateTo=(tab:Tab,targetId?:string)=>void;
...
type RolePermissionKey=
 ...
 "scheduleWorkout"|"enterCompetitionResult"|
...
const rolePermissions:Record<AccountRole,Record<RolePermissionKey,boolean>>={
 Player:{
  ...
  scheduleWorkout:true,enterCompetitionResult:true,
...
type TrackerProviderId="google-health"|"oura"|"whoop"|"strava"|"kinexon"|"garmin"|"apple-health"|"health-connect";
type TrackerProviderStatus={id:TrackerProviderId;name:string;kind:string;sleep:boolean;workouts:boolean;cloudOAuth:boolean;partnerApi?:boolean;note:string;configured:boolean};
...
type TrackerWorkoutMetric={provider:TrackerProviderId;provider_workout_id:string;workout_date:string;workout_type:string;duration_minutes?:number|null;avg_hr?:number|null;max_hr?:number|null;calories?:number|null;distance_meters?:number|null;strain?:number|null;high_speed_distance_meters?:number|null;sprint_distance_meters?:number|null;max_speed_mps?:number|null;accelerations?:number|null;decelerations?:number|null;player_load?:number|null};
type TrackerShareCategories={sleep:boolean;recovery:boolean;workouts:boolean;heartRate:boolean;movementLoad:boolean};
type TrackerCoachShare={coachUserId:string;coachName:string;teamNames:string[];active:boolean;categories:TrackerShareCategories;updatedAt?:string|null};
type TrackerDashboard={access?:"owner"|"coach";sharedCategories?:TrackerShareCategories;providers:TrackerProviderStatus[];connections:TrackerConnectionStatus[];daily:TrackerDailyMetric[];workouts:TrackerWorkoutMetric[]};
const fallbackTrackerProviders:TrackerProviderStatus[]=[];
...
export type BetaBridge={
 ...
 createAdminTestAthlete?:(input:{name:string;sport:string;age:number;position:string;team:string})=>Promise<{athleteId:string;workspaceId:string;name:string}>;
 openBetaAdmin?:()=>void;
 returnToCoachWorkspace?:()=>void;
 managedByParent?:boolean;
 juniorPlayerMode?:boolean;
...
const pageHelp:Record<string,{title:string;purpose:string;primary:string}>={
 Home:{title:"Overview",purpose:"What matters now.",primary:"Review today"},
 Goals:{title:"Goals",purpose:"Player-owned targets and progress.",primary:"Review goals"},
 Calendar:{title:"Schedule",purpose:"Training, events, and dates.",primary:"View schedule"},
 Testing:{title:"Testing",purpose:"Results, PRs, and retests.",primary:"Log a result"},
 Analytics:{title:"Progress",purpose:"Testing trends and development progress.",primary:"Review progress"},
 Coach:{title:"Readiness",purpose:"Recovery and daily context.",primary:"Review readiness"},
 Development:{title:"Development",purpose:"Priorities, skills, and next steps.",primary:"Review development"},
 Competition:{title:"Competition",purpose:"Results and learning.",primary:"Review competition"},
 Roster:{title:"Roster",purpose:"Athletes and connections.",primary:"Select a Player"}
};
const navMeta:Record<string,{icon:string;label:string}>={
 Home:{icon:"⌂",label:"Overview"},
 Goals:{icon:"goal" as PremiumIconName,label:"Goals"},
 Calendar:{icon:"calendar" as PremiumIconName,label:"Schedule"},
 Testing:{icon:"⚡",label:"Testing"},
 Analytics:{icon:"⌁",label:"Progress"},
 Coach:{icon:"✦",label:"Readiness"},
 Development:{icon:"development" as PremiumIconName,label:"Development"},
...
const sportHeroAsset=(sport:Sport)=>({
 ...
 "Ice Hockey":"/sport-heroes/ice-hockey.svg",
 "Basketball":"/sport-heroes/basketball.svg",
...
const realisticSportHeroAsset=(sport:Sport)=>({
 "Baseball":"/commercial-scenes/baseball-player.webp",
 "Football":"/commercial-scenes/football-player.webp",
 "Ice Hockey":"/commercial-scenes/ice-hockey-player.webp",
 "Basketball":"/commercial-scenes/basketball-player.webp",
...
function SmoothReadinessRing({value,label,status,statusClass}:{value:number|null;label:string;status:string;statusClass:string}){
 const progress=Math.max(0,Math.min(100,Number(value??0)));
 return <div className={`premiumReadinessOrb ${statusClass}`}>
  <svg className="premiumReadinessSvg" viewBox="0 0 120 120" aria-hidden="true">
   <circle className="premiumReadinessTrack" cx="60" cy="60" r="52" pathLength="100"/>
   <circle className="premiumReadinessProgress" cx="60" cy="60" r="52" pathLength="100" strokeDasharray={`${progress} ${100-progress}`}/>
  </svg>
  <div className="premiumReadinessContent">
   <small>{label}</small>
...
function PremiumAppIcon({name,className=""}:{name:PremiumIconName;className?:string}){
 ...
 const path=(()=>{
  switch(name){
   ...
   case "goal": return <><circle cx="12" cy="12" r="7.7"/><circle cx="12" cy="12" r="3.2"/><path d="M16.5 7.5 21 3m-4.5 4.5H21V3"/></>;
   case "calendar": return <><rect x="4" y="5.5" width="16" height="14" rx="2.5"/><path d="M8 3.5v4M16 3.5v4M4 9.5h16"/><path d="M8 13h2M14 13h2M8 16.5h2"/></>;
   ...
   case "competition": return <><path d="M8 4h8v5a4 4 0 0 1-8 0V4Z"/><path d="M8 6H4.5v2A4 4 0 0 0 8 12M16 6h3.5v2A4 4 0 0 1 16 12M12 13v4M8.5 20h7"/></>;
   case "roster": return <><circle cx="8" cy="9" r="3"/><circle cx="17" cy="8" r="2.3"/><path d="M3.5 19c.4-3.2 2.2-5 4.5-5s4.1 1.8 4.5 5M13.5 14.5c.8-1.5 2-2.3 3.5-2.3 2.2 0 3.6 1.5 3.9 4.3"/></>;
   case "support": return <><path d="M12 20s-7-4.3-7-10a4 4 0 0 1 7-2.7A4 4 0 0 1 19 10c0 5.7-7 10-7 10Z"/><path d="M9.2 11.5h5.6"/></>;
   case "recovery": return <><path d="M4 13.5c1.8-4.8 5.2-7 10.2-6.4"/><path d="M14.2 7.1 18 4.5l.4 4.7"/><path d="M20 10.5c-1.8 4.8-5.2 7-10.2 6.4"/><path d="M9.8 16.9 6 19.5l-.4-4.7"/></>;
...
const resizePlayerPhoto=(file:File)=>new Promise<string>((resolve,reject)=>{
 ...
 reader.onload=()=>{
  ...
  image.onload=()=>{
   ...
   const ctx=canvas.getContext("2d");if(!ctx){reject(new Error("Photo tools are unavailable in this browser."));return}
...
function PremiumHomeOverview({
 accountRole,juniorMode,profile,sport,goals,workouts,results,readiness,competitions,dev,setTab,onNavigate,trackerData,onOpenTrackers
}:{
 ...
 const readinessValue=latestReadiness?readinessScoreV2(latestReadiness,Number(profile.age||0)):null;
 const activeGoals=goals.filter(g=>(g.status||"Active")!=="Complete");
 const goalProgress=activeGoals.length?Math.round(activeGoals.reduce((sum,g)=>sum+Number(g.progress||0),0)/activeGoals.length):null;
 const nextWorkout=workouts.filter(w=>w.sport===sport&&!w.completed&&w.date>=today()).sort((a,b)=>a.date.localeCompare(b.date))[0];
 const nextCompetition=competitions.filter(c=>c.sport===sport&&c.date>=today()).sort((a,b)=>a.date.localeCompare(b.date))[0];
 const latestResult=results.filter(r=>r.sport===sport).slice().sort((a,b)=>b.date.localeCompare(a.date)||b.id-a.id)[0];
 const developmentFocus=dev.find(d=>d.status!=="Complete")?.title||"Build your next development focus";

 const roleCopy:Record<AccountRole,{eyebrow:string;title:string;subtitle:string}>={
  Player:{
   eyebrow:juniorMode?"MY ATHLETE HOME":"ATHLETE HOME",
   title:juniorMode?`Hi, ${profile.name||"Player"}!`:`${profile.name||"Athlete"}`,
   subtitle:juniorMode?"One thing at a time. Keep moving forward.":"Your training, development, and progress in one place."
  },
  Coach:{
   eyebrow:"COACH DEVELOPMENT",
   title:profile.name||"Player",
   subtitle:"Review the Player's current status, choose one development action, and move forward."
  },
  Parent:{
   eyebrow:"PARENT SUPPORT",
   title:profile.name||"Player",
   subtitle:"A calm snapshot of what matters and where support can help."
  },
  Admin:{
   eyebrow:"BETA OPERATIONS",
   title:profile.name||"Active Athlete",
   subtitle:"Test the product, inspect athlete data, and keep beta operations clean."
  }
 };
 ...
 const quickActions:Array<{icon:PremiumIconName;label:string;detail:string;tab:Tab;target?:string}>=
  ...
  :accountRole==="Coach"
  ?[
    {icon:"roster" as PremiumIconName,label:"Roster",detail:"Choose a Player",tab:"Roster" as Tab},
    {icon:"readiness" as PremiumIconName,label:"Readiness",detail:readinessValue!==null?`${readinessValue}/100 latest`:"No recent check-in",tab:"Coach" as Tab,target:"setup-readiness"},
    {icon:"development" as PremiumIconName,label:"Development",detail:developmentFocus,tab:"Development" as Tab},
    {icon:"goal" as PremiumIconName,label:"Player Goals",detail:activeGoals.length?`${activeGoals.length} active`:"No active goals",tab:"Goals" as Tab}
   ]
  :accountRole==="Parent"
  ?[
    {icon:"calendar" as PremiumIconName,label:"Schedule",detail:nextWorkout?nextWorkout.name:"See what is next",tab:"Calendar" as Tab},
    {icon:"recovery" as PremiumIconName,label:"Recovery",detail:readinessValue!==null?`${readinessValue}/100 latest`:"Review recovery",tab:"Coach" as Tab,target:"parent-recovery-summary"},
    {icon:"progress" as PremiumIconName,label:"Progress",detail:goalProgress!==null?`${goalProgress}% goal progress`:"See development trends",tab:"Analytics" as Tab},
    {icon:"support" as PremiumIconName,label:"Support",detail:developmentFocus,tab:"Development" as Tab}
   ]
  :[
    {icon:"roster" as PremiumIconName,label:"Roster",detail:"Athletes & connections",tab:"Roster" as Tab},
    {icon:"development" as PremiumIconName,label:"Development",detail:developmentFocus,tab:"Development" as Tab},
    {icon:"progress" as PremiumIconName,label:"Analytics",detail:"Review shared progress",tab:"Analytics" as Tab},
    {icon:"goal" as PremiumIconName,label:"Goals",detail:`${activeGoals.length} active`,tab:"Goals" as Tab}
   ];

 const statusLabel=readinessValue===null?"No check-in":readinessValue>=80?"Ready":readinessValue>=60?"Steady":"Recover";
 const statusClass=readinessValue===null?"neutral":readinessValue>=80?"good":readinessValue>=60?"steady":"watch";

 const roleFocus=accountRole==="Player"
  ?{
    eyebrow:juniorMode?"MY NEXT STEP":"TODAY'S FOCUS",
    title:nextWorkout?.name||developmentFocus,
    detail:nextWorkout?`${friendlyDate(nextWorkout.date)} · ${nextWorkout.minutes} min`:activeGoals[0]?`${activeGoals[0].title} · ${activeGoals[0].progress}% complete`:"Open Development to choose one clear priority.",
    tab:(nextWorkout?"Calendar":"Development") as Tab,
    action:nextWorkout?"Open training":"Open focus"
   }
  :accountRole==="Coach"
  ?{
    eyebrow:"COACH REVIEW",
    title:readinessValue!==null&&readinessValue<60?`Review ${profile.name||"this Player"}'s recovery`:developmentFocus,
    detail:readinessValue!==null?`Readiness ${readinessValue}/100 · ${activeGoals.length} active goal${activeGoals.length===1?"":"s"}`:"No recent readiness check-in · review the shared development record.",
    tab:(readinessValue!==null&&readinessValue<60?"Coach":"Development") as Tab,
    action:"Review Player"
   }
  :accountRole==="Parent"
  ?{
    eyebrow:"SUPPORT FOCUS",
    title:readinessValue!==null&&readinessValue<60?"Protect recovery and keep today simple":nextWorkout?`Help prepare for ${nextWorkout.name}`:developmentFocus,
    detail:readinessValue!==null&&readinessValue<60?`Readiness ${readinessValue}/100 · support sleep, hydration, transportation, and downtime.`:nextWorkout?`${friendlyDate(nextWorkout.date)} · practical support first.`:"Listen first and reinforce the Player/Coach plan.",
    tab:(readinessValue!==null&&readinessValue<60?"Coach":nextWorkout?"Calendar":"Development") as Tab,
    action:"Open support"
   }
  :{
    eyebrow:"BETA FOCUS",
    title:`Review ${profile.name||"the active athlete"}`,
    detail:`${sport} · ${results.filter(r=>r.sport===sport).length} test result${results.filter(r=>r.sport===sport).length===1?"":"s"} · ${activeGoals.length} active goal${activeGoals.length===1?"":"s"}`,
    tab:"Analytics" as Tab,
    action:"Open analytics"
   };

 const heroAsset=premiumHomeHeroAsset(sport,accountRole,juniorMode);
 const realisticHero=premiumHomeHeroIsRealistic(sport,juniorMode);

 const metricItems=[
  {label:"GOALS",value:goalProgress!==null?`${goalProgress}%`:"—",detail:activeGoals.length?`${activeGoals.length} active`:"Set a goal",tab:"Goals" as Tab,tone:"green"},
 ...
 const readinessState=readinessValue===null?null:readinessValue>=75?"Improving":readinessValue>=60?"Steady":"Needs Attention";
 const goalState=goalProgress===null?null:goalProgress>=70?"Improving":goalProgress>=40?"Steady":"Needs Attention";
 const progressState=latestResult===undefined?null:latestTestChange===null?"Steady":latestTestChange>2?"Improving":latestTestChange>=-2?"Steady":"Needs Attention";
 const trainingState=sportWorkouts.length===0?null:trainingConsistency>=75?"Improving":trainingConsistency>=50?"Steady":"Needs Attention";
 const signalTiles=[
  {key:"readiness",label:"READINESS",value:readinessValue!==null?String(readinessValue):"NO DATA",state:readinessState,detail:readinessValue!==null?"Latest check-in":"Add first check-in",progress:readinessValue??0,tab:"Coach" as Tab},
  {key:"goals",label:"GOAL EXECUTION",value:goalProgress!==null?`${goalProgress}%`:"NO DATA",state:goalState,detail:activeGoals.length?`${activeGoals.length} active goal${activeGoals.length===1?"":"s"}`:"Add first goal",progress:goalProgress??0,tab:"Goals" as Tab},
  {key:"testing",label:"PROGRESS",value:trendPrimary,state:progressState,detail:latestResult?trendTitle:"Add first test result",progress:latestTestChange===null?(latestResult?50:0):Math.max(0,Math.min(100,50+latestTestChange*2)),tab:"Analytics" as Tab},
  {key:"training",label:"TRAINING",value:sportWorkouts.length?`${trainingConsistency}%`:"NO DATA",state:trainingState,detail:nextWorkout?`${friendlyDate(nextWorkout.date)} · ${nextWorkout.name}`:sportWorkouts.length?`${completedSportWorkouts}/${sportWorkouts.length} complete`:"Add first training session",progress:trainingConsistency,tab:"Calendar" as Tab}
 ];
 const elitePerformanceBand=<div className="elitePerformanceBand rc54PerformanceTiles" aria-label="Athlete performance summary">
  {signalTiles.map(tile=><button type="button" key={tile.key} data-signal={tile.key} className={`rc54PerformanceTile ${tile.state?`hasData status${tile.state.replace(/\s/g,"")}`:"noData"}`} onClick={()=>onNavigate(tile.tab,tile.key==="readiness"?"setup-readiness":undefined)}><span className="eliteSignalTop"><small>{tile.label}</small><i className={`eliteStatusDot ${tile.state?tile.state.replace(/\s/g,"").toLowerCase():"neutral"}`}/></span><strong>{tile.value}</strong><span className="rc54TileStatus">{tile.state||"NO DATA"}</span><span className="rc54TileDetail">{tile.detail}</span><div className="eliteMicroGauge"><i style={{width:`${tile.progress}%`}}/></div></button>)}
 </div>;
 const eliteVisualPerformance=<button type="button" className="eliteVisualPerformance eliteProgressHome" onClick={()=>setTab("Analytics")} aria-label="Open Progress analytics">
  ...
  <div className="eliteVisualMetric"><strong>{trendPrimary}</strong><span>{latestResult?"latest test":"add first test result"}</span></div>
 ...
 const hero=<div className={`premiumHomeHero nativeSportsHero elitePerformanceHero rc34RoleHero ${realisticHero?"premiumRealisticSportHero":"premiumIllustratedSportHero"}`} data-hero-sport={sport} data-hero-role={accountRole} data-hero-style={realisticHero?"realistic":"illustrated"} style={{"--sport-hero-image":`url("${heroAsset}")`} as React.CSSProperties}>
  ...
  <div className="nativeHeroBottom">
   <div className="premiumHeroIdentity nativeHeroIdentity">
    ...
    <div><h1>{roleCopy[accountRole].title}</h1><p>{roleCopy[accountRole].subtitle}</p><div className="premiumAthleteMeta"><span>{sport}</span>{profile.position&&<span>{profile.position}</span>}{profile.team&&<span>{profile.team}</span>}</div></div>
   </div>
 ...
 const trackerHomeStrip=trackerConnected.length?<section className="trackerHomeStrip" aria-label="Connected tracker summary">
  ...
  <div><small>RECOVERY</small><b>{trackerDaily?.readiness_score!=null?`${Math.round(Number(trackerDaily.readiness_score))}/100`:trackerDaily?.hrv_ms!=null?`${Math.round(Number(trackerDaily.hrv_ms))} ms`:"—"}</b><span>{trackerDaily?.resting_hr!=null?`Resting HR ${Math.round(Number(trackerDaily.resting_hr))}`:accountRole==="Coach"?"Shared by Player / Parent":"Private unless you share"}</span></div>
 ...
 const trackerDiscoveryCta=(accountRole==="Player"||accountRole==="Parent"||accountRole==="Coach")&&onOpenTrackers?<button type="button" className="trackerDiscoveryCta" onClick={onOpenTrackers} aria-label={accountRole==="Coach"?"View Player-shared performance data":trackerConnected.length?"Manage connected performance trackers":"Connect a workout or sleep tracker"}>
  <span className="trackerDiscoveryIcon"><PremiumAppIcon name="recovery"/></span>
  <span className="trackerDiscoveryCopy"><small>{accountRole==="Coach"?"PLAYER-SHARED DATA":"WORKOUT + SLEEP DATA"}</small><b>{accountRole==="Coach"?"View Shared Performance":trackerConnected.length?"Manage Connected Trackers":"Connect a Performance Tracker"}</b><span>{accountRole==="Coach"?trackerConnected.length?`${trackerConnected.length} shared source${trackerConnected.length===1?"":"s"} · read only`:"Player / Parent controls category access":trackerConnected.length?`${trackerConnected.length} connected source${trackerConnected.length===1?"":"s"} · manage sync and privacy`:"Google Health, Oura, WHOOP, Strava and KINEXON"}</span></span>
  <strong>{accountRole==="Coach"?"VIEW →":trackerConnected.length?"MANAGE →":"CONNECT →"}</strong>
 </button>:null;
 ...
 if(accountRole==="Player"){
  return <section className="premiumHomeOverview nativeSportsHome nativePlayerHome" aria-label="Player home summary">
   ...
   <div className="nativePlayerFlow elitePlayerFlow">
    ...
    <button type="button" className="eliteRecoveryHome" onClick={()=>onNavigate("Coach","setup-readiness")} aria-label="Open Recovery tips and readiness">
...
function PlayerRoutinePriorityBanners({readiness,weeklyReviews,onNavigate}:{readiness:ReadinessLog[];weeklyReviews:WeeklyReview[];onNavigate:NavigateTo}){
 ...
 return <section className="routinePriorityStack" aria-label="Player actions to complete">
  {!dailyComplete&&<button type="button" className="routinePriorityBanner daily" onClick={()=>onNavigate("Coach","setup-readiness")}>
   <span className="routinePriorityIcon"><PremiumAppIcon name="readiness"/></span>
   <span className="routinePriorityCopy"><small>START HERE · ABOUT 1 MINUTE</small><b>Start Daily Check-In</b><span>Log sleep, energy, soreness, and stress.</span></span>
   <strong>CHECK IN →</strong>
  </button>}
  {!weeklyComplete&&<button type="button" className="routinePriorityBanner weekly" onClick={()=>onNavigate("Home","setup-weekly-review")}>
   <span className="routinePriorityIcon"><PremiumAppIcon name="progress"/></span>
...
const trackerShareOptions:{key:keyof TrackerShareCategories;label:string;detail:string}[]=[
 {key:"sleep",label:"Sleep",detail:"Sleep duration and sleep score"},
 {key:"recovery",label:"Recovery",detail:"Readiness and recovery scores"},
 {key:"workouts",label:"Workouts",detail:"Session type, duration, distance and calories"},
 {key:"heartRate",label:"Heart rate",detail:"Resting, average and maximum heart rate plus HRV"},
 {key:"movementLoad",label:"Movement & load",detail:"KINEXON speed, high-speed distance, accelerations and player load"}
];

function TrackerCoachSharingPanel({shares,loading,busyCoach,onSave}:{shares:TrackerCoachShare[];loading:boolean;busyCoach:string|null;onSave:(share:TrackerCoachShare)=>Promise<void>}){
 if(loading)return <div className="trackerLoading">Loading the Player's current Coaches and sharing choices…</div>;
 if(!shares.length)return <div className="trackerNoCoaches"><b>No Coach is connected yet</b><span>Join a Coach's team first. Sharing controls will appear here automatically.</span></div>;
 return <section className="trackerSharingPanel" aria-label="Coach tracker sharing">
  <div className="trackerProviderIntro"><div><small>OPT-IN COACH SHARING</small><b>Choose exactly what each Coach can see</b><span>Everything stays private until you turn sharing on. You can change or revoke access at any time.</span></div></div>
  <div className="trackerCoachShareList">{shares.map(share=><article key={share.coachUserId} className={`trackerCoachShareCard ${share.active?"active":""}`}>
   <div className="trackerCoachShareHead"><div><small>{share.teamNames.join(" · ")||"CONNECTED TEAM"}</small><b>{share.coachName}</b></div><label className="trackerMasterShare"><input type="checkbox" checked={share.active} disabled={busyCoach===share.coachUserId} onChange={e=>void onSave({...share,active:e.target.checked,categories:e.target.checked&&Object.values(share.categories).some(Boolean)?share.categories:e.target.checked?{sleep:false,recovery:false,workouts:true,heartRate:false,movementLoad:true}:share.categories})}/><span>{share.active?"Sharing on":"Sharing off"}</span></label></div>
   <div className="trackerShareCategoryGrid">{trackerShareOptions.map(option=><label key={option.key} className={share.categories[option.key]?"selected":""}><input type="checkbox" checked={share.categories[option.key]} disabled={busyCoach===share.coachUserId} onChange={e=>void onSave({...share,active:e.target.checked||share.active,categories:{...share.categories,[option.key]:e.target.checked}})}/><span><b>{option.label}</b><small>{option.detail}</small></span></label>)}</div>
   <p className="trackerShareStatus">{busyCoach===share.coachUserId?"Saving privacy choice…":share.active?`Coach can see: ${trackerShareOptions.filter(option=>share.categories[option.key]).map(option=>option.label).join(", ")||"no categories selected"}.`:"Coach cannot see imported tracker data."}</p>
  </article>)}</div>
  <div className="trackerPrivacyNote consent"><b>YOUR CHOICE</b><span>Sharing applies only to the selected categories and only while the Coach remains connected to this Player through a team. Tracker login credentials and provider account identifiers are never shared.</span></div>
 </section>;
}

function CoachSharedTrackerPanel({data,loading,message}:{data:TrackerDashboard|null;loading:boolean;message:string}){
 if(loading)return <div className="trackerLoading">Checking the Player's sharing permission…</div>;
 if(!data||data.access!=="coach")return <div className="trackerAccessLocked trackerPreviewLock"><b>PLAYER / PARENT OPT-IN REQUIRED</b><span>{message||"Ask the Player or a linked Parent to open Connected Trackers and choose which performance categories to share with you."}</span></div>;
 const d=data.daily?.[0],w=data.workouts?.[0],categories=data.sharedCategories;
 return <section className="coachSharedTrackerPanel">
  <div className="coachSharedTrackerBanner"><small>PLAYER-CONTROLLED ACCESS</small><h3>Shared performance data</h3><p>You can view only the categories the Player or linked Parent selected. You cannot connect, sync, disconnect, or change their privacy choices.</p></div>
  <div className="trackerSharedCategoryPills">{trackerShareOptions.filter(option=>categories?.[option.key]).map(option=><span key={option.key}>✓ {option.label}</span>)}</div>
  <div className="trackerSnapshot coachTrackerSnapshot"><div><small>LATEST SLEEP</small><b>{d?.sleep_minutes?`${Math.floor(Number(d.sleep_minutes)/60)}h ${Math.round(Number(d.sleep_minutes)%60)}m`:d?.sleep_score!=null?`${Math.round(Number(d.sleep_score))}%`:"Not shared"}</b></div><div><small>RECOVERY</small><b>{d?.readiness_score!=null?`${Math.round(Number(d.readiness_score))}/100`:d?.hrv_ms!=null?`${Math.round(Number(d.hrv_ms))} ms`:"Not shared"}</b></div><div><small>LAST SESSION</small><b>{w?.workout_type||"No shared session"}</b></div></div>
  {w&&<div className="kinexonPerformanceGrid"><div><small>DURATION</small><b>{w.duration_minutes!=null?`${Math.round(Number(w.duration_minutes))} min`:"—"}</b></div><div><small>DISTANCE</small><b>{w.distance_meters!=null?`${Math.round(Number(w.distance_meters))} m`:"—"}</b></div><div><small>PLAYER LOAD</small><b>{w.player_load!=null?Math.round(Number(w.player_load)):w.strain!=null?Number(w.strain).toFixed(1):"—"}</b></div><div><small>MAX SPEED</small><b>{w.max_speed_mps!=null?`${Number(w.max_speed_mps).toFixed(1)} m/s`:"—"}</b></div><div><small>HIGH SPEED</small><b>{w.high_speed_distance_meters!=null?`${Math.round(Number(w.high_speed_distance_meters))} m`:"—"}</b></div><div><small>ACCEL / DECEL</small><b>{w.accelerations!=null||w.decelerations!=null?`${Math.round(Number(w.accelerations||0))} / ${Math.round(Number(w.decelerations||0))}`:"—"}</b></div></div>}
  <div className="trackerProviderIntro"><div><small>SHARED SOURCES</small><b>{data.connections.length?data.connections.map(x=>data.providers.find(p=>p.id===x.provider)?.name||x.provider).join(" · "):"No imported records yet"}</b><span>{data.workouts.length||data.daily.length?"Newest shared records are shown above.":"The Player or Parent can sync their connected service when data is available."}</span></div></div>
  <div className="trackerPrivacyNote"><b>READ-ONLY COACH VIEW</b><span>Provider credentials, unselected categories, and account-management controls remain private to the Player and linked Parent.</span></div>
 </section>;
}
...
export default function AthleteApp({betaBridge}:{betaBridge?:BetaBridge}){
 ...
 const [results,setResults]=useState<Result[]>([]),[custom,setCustom]=useState<CustomTest[]>([]),[goals,setGoals]=useState<Goal[]>([]),[workouts,setWorkouts]=useState<Workout[]>([]),[profile,setProfile]=useState<Profile>({name:"Athlete",position:"",team:"",season:"2026-27",height:"",weight:"",handedness:"Right",age:"",sport:"Ice Hockey",photoUrl:""});
 ...
 const [trackerShares,setTrackerShares]=useState<TrackerCoachShare[]>([]);
 const [trackerSharesLoading,setTrackerSharesLoading]=useState(false);
 const [trackerShareBusyCoach,setTrackerShareBusyCoach]=useState<string|null>(null);
 const [kinexonAthleteId,setKinexonAthleteId]=useState("");
 ...
 const scrollToDestination=(targetId:string)=>{
  let attempts=0;
  const locate=()=>{
 ...
 const navigateTo:NavigateTo=(nextTab,targetId)=>{
  ...
  if(nextTab===tab){
   if(targetId)scrollToDestination(targetId);
 ...
 // Every navigation lands at its requested section. Generic tab changes still
 // start at the top so the user never arrives midway through an unrelated page.
 useEffect(()=>{
  if(!mounted)return;
  const targetId=pendingDestinationRef.current;
  pendingDestinationRef.current=null;
  const frame=window.requestAnimationFrame(()=>{
   if(targetId){scrollToDestination(targetId);return}
...
useEffect(()=>{if(program)localStorage.setItem("trainingProgram",JSON.stringify(program));else localStorage.removeItem("trainingProgram")},[program]);
...
},[betaBridge?.userId,betaBridge?.workspaceId,betaBridge?.session.role,betaBridge?.session.displayName]);
 ...
 useEffect(()=>{
   ...
   const syncNetwork=()=>{
     ...
     if(!online&&cloudLoadedRef.current){
 ...
 useEffect(()=>{
   ...
   const retry=()=>{
     setCloudOnline(true);
     if(cloudLoadedRef.current&&cloudReadyWorkspaceRef.current===betaBridge.workspaceId)void retryPendingCloudSave();
   };
   const offline=()=>{setCloudOnline(false);if(pendingCloudSave)setCloudStatus("waiting")};
   window.addEventListener("online",retry);
   window.addEventListener("offline",offline);
   return()=>{window.removeEventListener("online",retry);window.removeEventListener("offline",offline)};
 },[betaBridge?.workspaceId]);
 ...
 useEffect(()=>{
  ...
  setRoster(rows=>rows.map(r=>r.id===activeAthleteId?{
   ...
   season:profile.season,height:profile.height,weight:profile.weight,handedness:profile.handedness,age:profile.age??"",photoUrl:profile.photoUrl||""
  }:r));
 ...
 const playerAgeNumber=Number(profile.age||0);
 const juniorPlayerMode=effectiveRole==="Player"&&(
  betaBridge?.juniorPlayerMode===true||
 ...
 const trackerManageAllowed=Boolean(betaBridge&&trackerAthleteId&&(accountRole==="Player"||accountRole==="Parent"));
 const trackerCoachView=Boolean(betaBridge&&trackerAthleteId&&accountRole==="Coach");
 const trackerDiscoverable=TRACKER_CONNECTIVITY_ENABLED&&(effectiveRole==="Player"||effectiveRole==="Parent"||trackerCoachView);
 const trackerAllowed=TRACKER_CONNECTIVITY_ENABLED&&(trackerManageAllowed||trackerCoachView);
 const trackerProviders=trackerData?.providers?.length?trackerData.providers:fallbackTrackerProviders;
 const googleHealthConnection=trackerData?.connections?.find(x=>x.provider==="google-health");
 const googleHealthBusy=trackerBusyProvider==="google-health";
 const openTrackerCenter=()=>{
 ...
 const loadTrackers=async()=>{
  ...
  if(trackerManageAllowed)setTrackerSharesLoading(true);
  try{
   const token=await trackerAuthToken();const response=await fetch(`/api/trackers/status?athleteId=${encodeURIComponent(trackerAthleteId)}`,{headers:{Authorization:`Bearer ${token}`},cache:"no-store"});const json=await response.json();if(!response.ok)throw new Error(json.error||"Could not load trackers.");setTrackerData(json as TrackerDashboard);
   if(trackerManageAllowed){const shareResponse=await fetch(`/api/trackers/sharing?athleteId=${encodeURIComponent(trackerAthleteId)}`,{headers:{Authorization:`Bearer ${token}`},cache:"no-store"});const shareJson=await shareResponse.json();if(!shareResponse.ok)throw new Error(shareJson.error||"Could not load Coach sharing choices.");setTrackerShares(Array.isArray(shareJson.coaches)?shareJson.coaches:[])}
  }catch(error:any){setTrackerData(null);setTrackerMessage(error?.message||"Could not load tracker data.")}finally{setTrackerLoading(false);setTrackerSharesLoading(false)}
 };
 ...
 const connectKinexon=async()=>{
  if(!trackerManageAllowed)return;setTrackerBusyProvider("kinexon");setTrackerMessage("");
  try{const token=await trackerAuthToken();const response=await fetch("/api/trackers/kinexon/connect",{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${token}`},body:JSON.stringify({athleteId:trackerAthleteId,externalAthleteId:kinexonAthleteId})});const json=await response.json();if(!response.ok)throw new Error(json.error||"KINEXON could not be connected.");setTrackerMessage("KINEXON athlete linked. Sync now to import recent team performance sessions.");setKinexonAthleteId("");await loadTrackers()}catch(error:any){setTrackerMessage(error?.message||"KINEXON could not be connected.")}finally{setTrackerBusyProvider(null)}
 };
 const saveTrackerShare=async(share:TrackerCoachShare)=>{
  setTrackerShareBusyCoach(share.coachUserId);setTrackerMessage("");
  try{const token=await trackerAuthToken();const response=await fetch("/api/trackers/sharing",{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${token}`},body:JSON.stringify({athleteId:trackerAthleteId,coachUserId:share.coachUserId,active:share.active,categories:share.categories})});const json=await response.json();if(!response.ok)throw new Error(json.error||"Sharing choice could not be saved.");setTrackerShares(rows=>rows.map(row=>row.coachUserId===share.coachUserId?{...share,active:Boolean(json.active),categories:json.categories}:row));setTrackerMessage(json.active?`Sharing updated for ${share.coachName}.`:`Tracker sharing is off for ${share.coachName}.`)}catch(error:any){setTrackerMessage(error?.message||"Sharing choice could not be saved.")}finally{setTrackerShareBusyCoach(null)}
 };
 useEffect(()=>{if(trackerAllowed)void loadTrackers();else setTrackerData(null)},[trackerAllowed,trackerAthleteId]);
 useEffect(()=>{if(!mounted||!trackerAllowed)return;const query=new URLSearchParams(window.location.search);const tracker=query.get("tracker");if(!tracker)return;if(tracker==="connected"){setShowTrackerSetup(true);setTrackerMessage(`${query.get("provider")||"Tracker"} connected. Sync now to import recent data.`);void loadTrackers()}else if(tracker==="error"){setShowTrackerSetup(true);setTrackerMessage(query.get("message")||"Tracker connection failed.")}query.delete("tracker");query.delete("provider");query.delete("message");const next=query.toString()?`${window.location.pathname}?${query.toString()}`:window.location.pathname;window.history.replaceState({},"",next)},[mounted,trackerAllowed,trackerAthleteId]);
 ...
 const openNotice=(notice:InAppNotice)=>{
  ...
  if(notice.tab)navigateTo(notice.tab,notice.tab==="Coach"?"setup-readiness":undefined);
  setShowNotifications(false);
 };
 ...
 const openReadinessFromPrompt=()=>{
  setShowReadinessPrompt(false);
  navigateTo("Coach","setup-readiness");
 };
 ...
 const openWeeklyReviewFromPrompt=()=>{
  setShowWeeklyReviewPrompt(false);
  navigateTo("Home","setup-weekly-review");
 };

 const allAthletes=useMemo<AthleteRecord[]>(()=>{
  const current:AthleteRecord={id:activeAthleteId,name:profile.name,sport:profile.sport||sport,position:profile.position,team:profile.team,season:profile.season,height:profile.height,weight:profile.weight,handedness:profile.handedness,age:profile.age??"",photoUrl:profile.photoUrl||""};
 ...
 const coachGuideSteps:GuideStep[]=[
  commonWelcome,
  {id:"teams",title:"Invite Players & manage teams",body:"Roster is the main place to invite Players and manage team membership. Use Invite Player to share a team code; the Player or Parent creates and owns the Player account.",tab:"Roster",button:"Open Coach Roster"},
  ...
  {id:"finish",title:"Coach setup complete",body:"Your normal workflow is: scan the roster → review the Player → choose one development action → move to the next Player."}
 ];
 const parentGuideSteps:GuideStep[]=[
  ...
  {id:"calendar",title:"Use the schedule",body:"Schedule lets you add a support workout for the Player and review workouts, competitions, and important dates.",tab:"Calendar",button:"Open Schedule"},
 ...
 const adminGuideSteps:GuideStep[]=[
  commonWelcome,
  {id:"admin",title:"Open Beta Admin",body:"Beta Admin handles account access, Family & Account Diagnostics, safe relationship repairs, and feedback review.",action:"admin",button:"Open Beta Admin"},
 ...
 const jumpToGuideTarget=()=>{
  ...
  navigateTo(step.tab,step.id==="weekly"?"setup-weekly-review":step.target?`setup-${step.target}`:undefined);
  setGuideWaitingFor(step.id);
 ...
 const openFeatureFromHelp=(tab:Tab)=>{
  ...
  navigateTo(tab,tab==="Coach"&&effectiveRole==="Player"?"setup-readiness":tab==="Coach"&&effectiveRole==="Parent"?"parent-recovery-summary":undefined);
 };
 ...
 const roleNavLabel=(x:string)=>{
  if(effectiveRole==="Player"){
   if(juniorPlayerMode){
    ...
    if(x==="Goals")return "My Goal";
    if(x==="Calendar")return "My Training";
    if(x==="Coach")return "How I Feel";
    if(x==="Testing")return "My Tests";
    if(x==="Analytics")return "How I'm Doing";
    if(x==="Development")return "My Skills";
    if(x==="Competition")return "Games";
   }
   if(x==="Home")return "Today";
 ...
 const playerPageHelp:Partial<Record<Tab,{title:string;purpose:string;primary:string}>>=juniorPlayerMode?{
  Home:{title:"Today",purpose:"One thing at a time.",primary:"Start here"},
  Goals:{title:"My Goal",purpose:"One clear goal.",primary:"See my goal"},
  Calendar:{title:"My Training",purpose:"What is next.",primary:"Open next"},
  Coach:{title:"How I Feel",purpose:"Body and energy today.",primary:"Check in"},
  Development:{title:"My Skills",purpose:"What I am working on.",primary:"See my focus"},
  Testing:{title:"My Tests",purpose:"My latest results.",primary:"See results"},
  Analytics:{title:"How I'm Doing",purpose:"Wins and progress.",primary:"See progress"},
  Competition:{title:"Games",purpose:"Games and events.",primary:"Open latest"}
}:{
  Home:{title:"Today",purpose:"Your next important actions.",primary:"Start with one"},
  Goals:{title:"My Goals",purpose:"Current targets and progress.",primary:"Review a goal"},
  Calendar:{title:"My Schedule",purpose:"Training and events.",primary:"Open next"},
  Coach:{title:"Daily Check-In",purpose:"Sleep, energy, soreness, stress.",primary:"Check in"},
  Development:{title:"My Development",purpose:"Current focus and next step.",primary:"Review focus"},
  Testing:{title:"My Testing",purpose:"Results, PRs, and retests.",primary:"Review results"},
  Analytics:{title:"My Progress",purpose:"Your development trends.",primary:"Read summary"},
  Competition:{title:"Competition",purpose:"Results and learning.",primary:"Open latest"}
 };
 const parentPageHelp:Partial<Record<Tab,{title:string;purpose:string;primary:string}>>={
  Home:{title:"Parent Overview",purpose:"What matters this week.",primary:"Review support"},
  Calendar:{title:"Schedule",purpose:"Training and events.",primary:"Check next"},
  Coach:{title:"Recovery & Notes",purpose:"Readiness and communication.",primary:"Review recovery"},
  Analytics:{title:"Progress",purpose:"Development trends.",primary:"Read summary"},
  Development:{title:"Development",purpose:"How to help right now.",primary:"Review focus"},
  Competition:{title:"Competition",purpose:"Results and learning.",primary:"Review latest"}
 };
 ...
 const openNavGroup=(group:keyof typeof navGroups)=>{
  ...
  // Player More is a gateway, not a one-tab shortcut. Keep the sheet open
  // even when Competition is the only direct More tab so Search All Features
  // remains available.
  if(group==="More"&&effectiveRole==="Player"){setNavSheet("More");return}
 ...
 const featureCatalog:{tab:Tab;group:"Overview"|"Plan"|"Train"|"Progress"|"More";title:string;description:string;how:string}[]=[
  ...
  {tab:"Analytics",group:"Progress",title:effectiveRole==="Player"?"My Progress":effectiveRole==="Parent"?"Progress":"Analytics & Reports",description:effectiveRole==="Player"?"A simple summary of whether training, testing, goals, readiness, and competition are moving in the right direction.":effectiveRole==="Parent"?"Review the clearest progress trends without editing Player data.":"Review shared results, trends, reports, and next steps.",how:effectiveRole==="Player"?"Open Progress, then My Progress.":effectiveRole==="Parent"?"Tap Progress at the bottom.":"Open Progress, then Analytics."},
  {tab:"Competition",group:"More",title:"Competition",description:effectiveRole==="Player"?"Log and review games, matches, meets, ratings, confidence, and sport-specific stats.":effectiveRole==="Parent"?"Enter a factual score/result and review competition history without writing the Player's reflection.":"Log and review competition results, notes, ratings, and sport-specific statistics.",how:"Open More, then Competition."},
 ...
 const roleHelpTasks:RoleHelpTask[]=effectiveRole==="Player"?[
  ...
  {title:"How do I schedule a workout?",detail:"Select the Player, then open Schedule and use Schedule Workout.",tab:"Calendar"},
  {title:"How do I enter a competition result?",detail:"Select the Player, then open Competition and log the event result.",tab:"Competition"},
  {title:"How do I invite a Player?",detail:"Open Roster, then tap Invite Player. Select a team and copy the invite message or code.",tab:"Roster"},
  ...
  {title:"How do I schedule a workout?",detail:"Open Schedule and use the Parent Support scheduling card.",tab:"Calendar"},
  {title:"How do I enter a competition score?",detail:"Open Competition and use Add Competition Score / Result.",tab:"Competition"},
 ...
 const openRoleHelpTask=(task:RoleHelpTask)=>{
  ...
  navigateTo(task.tab,task.subView==="weekly"?"setup-weekly-review":task.tab==="Coach"&&effectiveRole==="Player"?"setup-readiness":task.tab==="Coach"&&effectiveRole==="Parent"?"parent-recovery-summary":undefined);
 };
 ...
 const quickActions:QuickAction[]=[
  {id:"home",label:juniorPlayerMode?"Today":effectiveRole==="Player"?"Go to Today":"Go to Overview",tab:"Home",keywords:["dashboard","home","today","summary"]},
  {id:"goals",label:juniorPlayerMode?"My Goal":effectiveRole==="Player"?"Open My Goals":"Open Goals",tab:"Goals",keywords:["goal","target","deadline"]},
  {id:"calendar",label:juniorPlayerMode?"Training":effectiveRole==="Player"?"Open My Schedule":"Open Schedule",tab:"Calendar",keywords:["workout","schedule","training"]},
  {id:"testing",label:juniorPlayerMode?"My Tests":effectiveRole==="Player"?"Open My Testing":"Open Testing",tab:"Testing",keywords:["test","pr","retest"]},
  {id:"analytics",label:juniorPlayerMode?"How I'm Doing":effectiveRole==="Player"?"Open My Progress":"Open Analytics",tab:"Analytics",keywords:["trend","report","scorecard","progress"]},
  {id:"coach",label:juniorPlayerMode?"How I Feel":effectiveRole==="Player"?"Open Daily Check-In":effectiveRole==="Parent"?"Open Recovery":"Open Readiness & Coach Tools",tab:"Coach",keywords:["readiness","recommendation","recovery","feel"],target:effectiveRole==="Player"?"setup-readiness":effectiveRole==="Parent"?"parent-recovery-summary":undefined},
  {id:"development",label:juniorPlayerMode?"My Skills":effectiveRole==="Player"?"Open My Development":effectiveRole==="Parent"?"Open Development Support":"Open Development",tab:"Development",keywords:["development","milestone","priority","objective","skills"]},
  {id:"competition",label:juniorPlayerMode?"Games":"Open Competition",tab:"Competition",keywords:["game","match","stats"]},
 ...
 const openManagedPlayerProfile=()=>{
  setCommandOpen(false);
  setNavSheet(null);
  setShowSettings(false);
  setShowNotifications(false);
  setTab("Home");
  // Handle both an already-mounted Home page and a Home page that mounts
  // after navigating from another Junior tab.
  setEditProfileRequest(x=>x+1);
  window.setTimeout(()=>setEditProfileRequest(x=>x+1),320);
  window.setTimeout(()=>{
   const profileCard=document.getElementById("setup-profile");
   const editButton=profileCard?.querySelector<HTMLButtonElement>(".profileEditButton");
 ...
 return <div className="app performanceOS" data-text-size={textSize} data-role={effectiveRole} data-junior={juniorPlayerMode?"true":"false"} data-tab={tab} data-sport={sport}><a className="skipLink" href="#main-content">Skip to main content</a>
  ...
  {accountRole==="Admin"&&<div className="adminPreviewBar" role="region" aria-label="Admin preview controls">
    <div className="adminPreviewIdentity">
      ...
      <b>Previewing {adminView}{adminView==="Player"&&juniorPlayerMode?" · Junior":""}</b>
      <span>Signed in as Admin · preview controls stay available in every role.</span>
    </div>
    <div className="adminPreviewControls">
      {allowedAthletes.length>0&&<label className="adminPreviewAthlete"><small>Athlete</small><select value={activeAthleteId} onChange={e=>{selectAthleteById(e.target.value);setTab("Home")}}>{allowedAthletes.map(a=><option value={a.id} key={a.id}>{a.name} · {a.sport}{a.team?` · ${a.team}`:""}</option>)}</select></label>}
      <label className="adminPreviewRole"><small>Preview role</small><select value={adminView} onChange={e=>{setAdminView(e.target.value as "Admin"|"Coach"|"Player"|"Parent");setTab("Home")}}><option>Admin</option><option>Coach</option><option>Player</option><option>Parent</option></select></label>
      {adminView!=="Admin"&&<button type="button" className="adminPreviewReturn" onClick={()=>{setAdminView("Admin");setTab("Home")}}>Return to Admin</button>}
      {betaBridge?.openBetaAdmin&&<button type="button" className="adminPreviewBetaAdmin" onClick={betaBridge.openBetaAdmin}>Beta Admin</button>}
    </div>
  </div>}
  {juniorPlayerMode&&tab!=="Home"&&<div className="juniorReturnBar"><button type="button" onClick={()=>setTab("Home")}>← Today</button><b>{roleNavLabel(tab)}</b></div>}
  {betaBridge&&cloudStatus!=="saved"&&<div className={"cloudSyncNotice "+cloudStatus} role="status">
    <div><span>{cloudStatus==="loading"?"↻":cloudStatus==="waiting"?"⌁":cloudStatus==="error"?"!":"•"}</span><div><b>{cloudStatus==="loading"?"Saving your changes…":cloudStatus==="waiting"?"Changes are waiting for connection":cloudStatus==="error"?"Cloud save needs attention":"Cloud sync"}</b><small>{cloudStatus==="waiting"?"Your work is saved on this device and will retry when internet returns.":cloudStatus==="error"?(cloudErrorMessage||"A retry copy is saved on this device."):"Keep using the app — this will finish automatically."}</small></div></div>
    {(cloudStatus==="waiting"||cloudStatus==="error")&&<button type="button" onClick={()=>void retryPendingCloudSave()} disabled={!cloudOnline}>Retry Now</button>}
   </div>}
  ...
  {showTrackerSetup&&<ViewportPortal><div className="trackerSetupOverlay" role="dialog" aria-modal="true" aria-label="Connected tracker setup" onClick={()=>setShowTrackerSetup(false)}><div className="trackerSetupScreen" onClick={e=>e.stopPropagation()}>
   <div className="trackerSetupHead"><div><small>{accountRole==="Coach"?"PLAYER-SHARED PERFORMANCE":"CONNECTED TRACKERS"}</small><h2>{accountRole==="Coach"?"Shared tracker & performance data":"Connect workout, sleep & performance data"}</h2><p>{accountRole==="Coach"?"This read-only view follows the Player or Parent's opt-in category choices.":"Choose a service below. Google Health is recommended for Fitbit and Pixel Watch; KINEXON uses team-issued partner access."}</p></div><button type="button" aria-label="Close tracker setup" onClick={()=>setShowTrackerSetup(false)}>×</button></div>
   {accountRole==="Admin"?<div className="trackerAccessLocked trackerPreviewLock"><b>PLAYER / PARENT SIGN-IN REQUIRED</b><span>Tracker authorization and Player-controlled Coach sharing cannot be changed from an Admin preview.</span></div>:accountRole==="Coach"?<CoachSharedTrackerPanel data={trackerData} loading={trackerLoading} message={trackerMessage}/>:!trackerAthleteId?<div className="trackerAccessLocked"><b>{accountRole==="Parent"?"CHOOSE A PLAYER":"PLAYER CLOUD RECORD REQUIRED"}</b><span>{accountRole==="Parent"?"Select the Player whose private workout and sleep data you want to connect.":"This Player login is not yet linked to its canonical cloud athlete record. Complete Player setup, then return here."}</span>{accountRole==="Parent"&&betaBridge?.openParentPlayers?<button type="button" className="featureAction trackerChoosePlayer" onClick={()=>{setShowTrackerSetup(false);betaBridge.openParentPlayers?.()}}>Choose Player</button>:<button type="button" className="featureAction trackerChoosePlayer" onClick={()=>{setShowTrackerSetup(false);setGuideStep(0);setShowGuide(true)}}>Open Player Setup</button>}</div>:<>
    ...
    {trackerLoading&&!trackerData?<div className="trackerLoading">Checking current connections… The Connect Google Health button above is ready to use.</div>:null}
    ...
    <div className="trackerProviderIntro"><div><small>OTHER SERVICES</small><b>Connect another tracker</b><span>Each provider opens its own secure authorization flow when credentials are enabled.</span></div></div>
    <div className="trackerProviderGrid trackerSetupProviderGrid">{trackerProviders.filter(provider=>provider.id!=="google-health").map(provider=>{const connection=trackerData?.connections?.find(x=>x.provider===provider.id);const busy=trackerBusyProvider===provider.id;return <article key={provider.id} className={`trackerProviderCard ${connection?.status||""}`}><div className="trackerProviderHead"><span className={`trackerProviderMark ${provider.id}`}>{provider.name.slice(0,1)}</span><div><b>{provider.name}</b><small>{provider.kind}</small></div><span className={`trackerStatus ${connection?.status==="connected"?"connected":provider.configured?"ready":"setup"}`}>{connection?.status==="connected"?"Connected":provider.configured?"Ready":"Setup needed"}</span></div><p>{provider.note}</p><div className="trackerProviderActions">{connection?.status==="connected"?<><button type="button" disabled={busy} className="featureAction" onClick={()=>void syncTracker(provider.id)}>{busy?"Syncing…":"Sync now"}</button><button type="button" disabled={busy} onClick={()=>void disconnectTracker(provider.id)}>Disconnect</button></>:provider.id==="kinexon"?<div className="kinexonConnectForm"><input value={kinexonAthleteId} disabled={busy||!provider.configured} onChange={e=>setKinexonAthleteId(e.target.value)} placeholder="KINEXON athlete ID" aria-label="KINEXON athlete ID"/><button type="button" disabled={busy||!provider.configured||!kinexonAthleteId.trim()} className="featureAction" onClick={()=>void connectKinexon()}>{busy?"Linking…":provider.configured?"Link KINEXON →":"KINEXON partner setup needed"}</button><small>{provider.configured?"Use the athlete ID supplied by the Player's KINEXON team.":"Your deployment owner must first add the KINEXON-issued API details."}</small></div>:provider.cloudOAuth?<button type="button" disabled={busy||!provider.configured} className="featureAction" onClick={()=>void connectTracker(provider.id)}>{busy?"Opening…":provider.configured?`Connect ${provider.name} →`:`${provider.name} setup needed`}</button>:<button type="button" disabled>Future mobile / partner connection</button>}</div></article>})}</div>
    <TrackerCoachSharingPanel shares={trackerShares} loading={trackerSharesLoading} busyCoach={trackerShareBusyCoach} onSave={saveTrackerShare}/>
    <div className="trackerPrivacyNote"><b>PRIVATE BY DEFAULT</b><span>The signed-in Player and linked Parent can always see imported tracker metrics. A Coach sees nothing unless the Player or Parent explicitly turns on selected categories above. Admin remains blocked.</span></div>
   </>}
   <div className="trackerSetupFooter"><button type="button" onClick={()=>{setShowTrackerSetup(false);setShowSettings(true)}}>Back to Settings</button><button type="button" className="featureAction" onClick={()=>setShowTrackerSetup(false)}>Done</button></div>
  </div></div></ViewportPortal>}
  {showSettings&&<ViewportPortal><div className="settingsOverlay viewportSettingsOverlay" role="dialog" aria-modal="true" aria-label="App settings" onClick={()=>setShowSettings(false)}><div className="settingsCard" onClick={e=>e.stopPropagation()}>
   ...
   <div className="notificationSettings">
    <div className="settingLabel"><b>Performance alerts</b><span>Choose which updates appear in the Alerts center.</span></div>
   ...
   {trackerDiscoverable&&<div className="trackerSettings" id="connected-trackers-settings">
    <div className="settingLabel"><b>Connected trackers</b><span>Workout, sleep and recovery connections now open in a dedicated setup screen so the actual connection controls are easy to find.</span></div>
...
function RolePermissionAuditPanel(){
 const rows:{label:string;key:RolePermissionKey}[]=[
  ...
  {label:"Schedule Workout",key:"scheduleWorkout"},
  {label:"Enter Competition Result",key:"enterCompetitionResult"},
 ...
 return <>
  ...
  <div className="card sharedDevelopmentFocusCard parentFocusCard">
   ...
   <div className="sharedFocusGrid">
    ...
    <div><small>PARENT SUPPORT</small><b>{parentSupportHeadline}</b><span>{parentSupportDetail}</span></div>
   </div>
  </div>
...
function ParentSchedule({sport,workouts,setWorkouts,competitions,seasonEvents,setTab}:{sport:Sport;workouts:Workout[];setWorkouts:React.Dispatch<React.SetStateAction<Workout[]>>;competitions:CompetitionLog[];seasonEvents:SeasonEvent[];setTab:React.Dispatch<React.SetStateAction<Tab>>}){
 const canSchedule=canRole("Parent","scheduleWorkout");
 const [workoutDate,setWorkoutDate]=useState(today()),[workoutName,setWorkoutName]=useState(""),[workoutCategory,setWorkoutCategory]=useState("Sport Practice"),[workoutMinutes,setWorkoutMinutes]=useState("45"),[workoutIntensity,setWorkoutIntensity]=useState<"Easy"|"Moderate"|"Hard">("Moderate"),[workoutFocus,setWorkoutFocus]=useState("");
 const [scheduleMessage,setScheduleMessage]=useState("");
 const scheduleWorkout=()=>{
  if(!canSchedule||!workoutName.trim())return;
  const item:Workout={id:Date.now(),date:workoutDate,name:workoutName.trim(),category:workoutCategory,minutes:Number(workoutMinutes)||45,completed:false,sport,intensity:workoutIntensity,focus:workoutFocus.trim(),source:"Manual",assignedByRole:"Parent"};
  setWorkouts(rows=>[item,...rows]);
  setWorkoutName("");setWorkoutFocus("");
  setScheduleMessage("Workout scheduled for the Player.");
 };
 ...
 return <>
  ...
  <div className="hero"><small>PARENT · SCHEDULE</small><h1>What's Coming Up?</h1><p>Schedule a workout for the Player, or review what is already coming up.</p></div>
  <div className="card parentScheduleWorkoutCard">
   <div className="sectionHead"><div><small>PARENT SUPPORT</small><h2>Schedule a Workout</h2><p>Parents can put a workout on the Player's schedule. This does not change the Coach-owned Development Plan or create practice plans.</p></div><span className="tag">SCHEDULE ONLY</span></div>
   <div className="two">
    <label>Date<input type="date" value={workoutDate} onChange={e=>setWorkoutDate(e.target.value)}/></label>
    <label>Workout name<input value={workoutName} onChange={e=>setWorkoutName(e.target.value)} placeholder="e.g. Off-ice workout"/></label>
    <label>Category<select value={workoutCategory} onChange={e=>setWorkoutCategory(e.target.value)}><option>Sport Practice</option><option>Strength</option><option>Speed</option><option>Conditioning</option><option>Mobility</option><option>Recovery</option><option>Skills</option><option>Other</option></select></label>
    <label>Duration<select value={workoutMinutes} onChange={e=>setWorkoutMinutes(e.target.value)}>{["15","30","45","60","75","90","120"].map(x=><option key={x} value={x}>{x} minutes</option>)}</select></label>
    <label>Intensity<select value={workoutIntensity} onChange={e=>setWorkoutIntensity(e.target.value as "Easy"|"Moderate"|"Hard")}><option>Easy</option><option>Moderate</option><option>Hard</option></select></label>
    <label>Focus / note<input value={workoutFocus} onChange={e=>setWorkoutFocus(e.target.value)} placeholder="Optional"/></label>
   </div>
   <button className="featureAction parentScheduleWorkoutButton" disabled={!canSchedule||!workoutName.trim()} onClick={scheduleWorkout}>Schedule Workout</button>
   {scheduleMessage&&<div className="betaMessage">{scheduleMessage}</div>}
  </div>
...
function ParentCompetition({sport,profile,competitions,setCompetitions,setTab}:{sport:Sport;profile:Profile;competitions:CompetitionLog[];setCompetitions:React.Dispatch<React.SetStateAction<CompetitionLog[]>>;setTab:React.Dispatch<React.SetStateAction<Tab>>}){
 const canEnterResult=canRole("Parent","enterCompetitionResult");
 const [resultDate,setResultDate]=useState(today()),[resultType,setResultType]=useState("Game"),[resultOpponent,setResultOpponent]=useState(""),[resultScore,setResultScore]=useState(""),[resultLocation,setResultLocation]=useState("");
 const [resultMessage,setResultMessage]=useState("");
 const addCompetitionResult=()=>{
  if(!canEnterResult||(!resultOpponent.trim()&&!resultScore.trim()))return;
  const item:CompetitionLog={id:Date.now(),date:resultDate,opponent:resultOpponent.trim()||"Competition",eventType:resultType,result:resultScore.trim(),minutes:"",rating:0,notes:"",sport,stats:[],location:resultLocation.trim(),role:profile.position||"",confidence:0,enteredByRole:"Parent",entryKind:"Score"};
  setCompetitions(rows=>rows.some(c=>c.sport===sport&&c.date===item.date&&c.opponent===item.opponent&&c.result===item.result)?rows:[item,...rows]);
  setResultOpponent("");setResultScore("");setResultLocation("");
  setResultMessage("Competition result saved.");
 };
 ...
 return <>
  ...
  <div className="hero"><small>PARENT · COMPETITION</small><h1>Competition Results</h1><p>{profile.name} · Parents can enter the score/result. Player reflections and Coach observations remain owned by those roles.</p></div>
  <div className="card parentCompetitionResultCard">
   <div className="sectionHead"><div><small>PARENT SUPPORT</small><h2>Add Competition Score / Result</h2><p>Record the factual event result without creating a Player performance rating or reflection.</p></div><span className="tag">RESULT ONLY</span></div>
   <div className="two">
    <label>Date<input type="date" value={resultDate} onChange={e=>setResultDate(e.target.value)}/></label>
    <label>Type<select value={resultType} onChange={e=>setResultType(e.target.value)}><option>Game</option><option>Match</option><option>Tournament</option><option>Scrimmage</option><option>Meet</option><option>Showcase</option></select></label>
    <label>Opponent / Event<input value={resultOpponent} onChange={e=>setResultOpponent(e.target.value)} placeholder="Team or event name"/></label>
    <label>Score / Result<input value={resultScore} onChange={e=>setResultScore(e.target.value)} placeholder="e.g. W 4-2, 2nd place"/></label>
    <label>Location<input value={resultLocation} onChange={e=>setResultLocation(e.target.value)} placeholder="Optional"/></label>
   </div>
   <button className="featureAction parentCompetitionResultButton" disabled={!canEnterResult||(!resultOpponent.trim()&&!resultScore.trim())} onClick={addCompetitionResult}>Save Competition Result</button>
   {resultMessage&&<div className="betaMessage">{resultMessage}</div>}
  </div>
  ...
  <div className="card parentCompetitionCard">
   {visible.length===0?<div className="parentEmptyState"><b>No competitions logged yet</b><span>Competition history will appear after the athlete or coach logs an event.</span></div>:visible.map(c=><div className="parentExpandable" key={c.id}>
    <button onClick={()=>setExpanded(expanded===c.id?null:c.id)} aria-expanded={expanded===c.id}><span className="parentDateBadge">{c.date.slice(5)}</span><div><small>{c.eventType.toUpperCase()}</small><b>{c.opponent||"Competition"}</b><span>{c.result||"Result not entered"}{c.rating>0?` · ${c.rating}/10`:" · Result entered"}</span></div><strong>{expanded===c.id?"−":"+"}</strong></button>
    {expanded===c.id&&<div className="parentExpandedDetail competition">
     ...
     {c.enteredByRole&&<p className="muted"><b>Entered by:</b> {c.enteredByRole}{c.entryKind==="Score"?" · score/result only":""}</p>}
...
function PlayerProgressSimple({sport,profile,results,goals,workouts,readiness,competitions,onDetails,setTab}:{sport:Sport;profile:Profile;results:Result[];goals:Goal[];workouts:Workout[];readiness:ReadinessLog[];competitions:CompetitionLog[];onDetails:()=>void;setTab:React.Dispatch<React.SetStateAction<Tab>>}){
 ...
 const competition=ratedComps.length?Math.round(ratedComps.reduce((sum,c)=>sum+c.rating,0)/ratedComps.length*10):0;
 ...
 return <div className="playerProgressSimple">
  ...
  <div className="playerProgressSnapshot">
   <div className="playerProgressMain">
    <small>OVERALL</small><h2>{status}</h2><p>{overall>=80?"Your current development results are strong. Keep doing the things that are working.":overall>=60?"You are building. Keep the routine consistent and focus on the clearest next step.":overall>=40?"Some areas are moving well and some need more attention. Focus on one thing at a time.":"You need more data or one area needs attention. Use the next step below rather than chasing every number."}</p>
   </div>
...
function CoachHomeDevelopmentDashboard({roster,status,selectedAthleteName,selectAthlete,setTab,openTeams}:{roster:CoachCloudAthleteState[];status:"idle"|"loading"|"ready"|"error";selectedAthleteName?:string;selectAthlete?:((workspaceId:string)=>void);setTab:React.Dispatch<React.SetStateAction<Tab>>;openTeams?:()=>void}){
 ...
 const rows=roster.map(c=>{
  ...
  const athleteWorkouts=(Array.isArray(raw.workouts)?raw.workouts:[]) as Workout[];
  const nextWorkout=athleteWorkouts.filter(w=>w.sport===athleteSport&&!w.completed&&w.date>=today()).sort((a,b)=>a.date.localeCompare(b.date))[0];
  ...
  return {
   ...
   photoUrl:profile.photoUrl||"",readinessScore,readinessAvailable:recentReadiness.length>0,planNeedsReview,goalsNeedFeedback,
   ...
   nextSession:nextWorkout?.name||"",nextSessionDate:nextWorkout?.date||""
  };
 ...
 const readinessAvailableRows=rows.filter(x=>x.readinessAvailable);
 const teamReadiness=readinessAvailableRows.length?Math.round(readinessAvailableRows.reduce((sum,x)=>sum+x.readinessScore,0)/readinessAvailableRows.length):0;
 const upcomingSessions=rows.filter(x=>x.nextSessionDate).length;
 ...
 return <section className="coachHomeCommandCenterV2">
  <div className="coachHomeV2Head">
   ...
   <div className="coachHomeV2Actions">{openTeams&&<button onClick={openTeams}>Teams & Invites</button>}<button className="featureAction" onClick={()=>setTab("Roster")}>Open Full Command Center</button></div>
  </div>

  <div className="coachHomeV2Metrics">
   <div><small>PLAYERS</small><b>{rows.length}</b><span>managed athletes</span></div>
   <div className="coachMetricReadiness"><small>TEAM READINESS</small><b>{teamReadiness||"—"}</b><span>{readinessAvailableRows.length?`${readinessAvailableRows.length} reporting`:"No check-ins"}</span></div>
   <div><small>UPCOMING</small><b>{upcomingSessions}</b><span>scheduled sessions</span></div>
  ...
  {nextPlayer&&<div className="coachReviewNext">
   <div className="coachReviewNextHead"><div className="coachNextPlayerIdentity"><PlayerPhotoAvatar name={nextPlayer.name} photoUrl={nextPlayer.photoUrl} size={58}/><div><small>WHO SHOULD I REVIEW NEXT?</small><h2>{nextPlayer.name}</h2><p>{nextPlayer.sport}{nextPlayer.position?` · ${nextPlayer.position}`:""}{selectedAthleteName===nextPlayer.name?" · currently selected":""}</p></div></div><span className="tag">{nextPlayer.points} PRIORITY POINTS</span></div>
   <div className="coachReviewNextBody">
    ...
    <div><small>NEXT BEST ACTION</small><b>{nextPlayer.nextAction}</b><span>Current focus: {nextPlayer.primaryFocus}</span>{nextPlayer.nextSession&&<span className="coachNextSession">Next session: {nextPlayer.nextSession} · {friendlyDate(nextPlayer.nextSessionDate)}</span>}</div>
   </div>
   <div className="coachReviewNextActions"><button onClick={()=>openPlayer(nextPlayer,"Goals")}>Player Goals</button><button onClick={()=>openPlayer(nextPlayer,"Calendar")}>Schedule Workout</button><button onClick={()=>openPlayer(nextPlayer,"Competition")}>Add Competition Result</button><button onClick={()=>openPlayer(nextPlayer,"Analytics")}>Progress</button><button onClick={()=>openPlayer(nextPlayer,"Development")}>Development</button><button className="featureAction" onClick={()=>openPlayer(nextPlayer,nextPlayer.nextTab,nextPlayer.nextSubView)}>Do Next Action</button></div>
  </div>}
  ...
  {rows.length>0&&<div className="coachHomePriorityQueue">
   ...
   {rows.slice(0,5).map((row,index)=><div className={"coachHomePriorityRow "+(row.points>=6?"high":row.points>=3?"medium":"current")} key={row.workspaceId}>
    ...
    <PlayerPhotoAvatar name={row.name} photoUrl={row.photoUrl} size={42} className="coachQueueAvatar"/>
    <div><b>{row.name}</b><small>{row.sport}{row.position?` · ${row.position}`:""}{row.nextSessionDate?` · ${friendlyDate(row.nextSessionDate)}`:""}</small></div>
...
function Home({accountRole,juniorMode,sport,setSport,goals,workouts,results,profile,setProfile,onProfileSaved,readiness,competitions,dev,program,weeklyReviews,setWeeklyReviews,coachWeeklyReviews,developmentSystem,testTargets,workspaceRole,onboardingDismissed,setOnboardingDismissed,setTab,onNavigate,editProfileRequest,openCoachTeams,coachSelectedAthleteName,loginSessionKey,coachCloudRoster,coachRosterCloudStatus,selectCoachRosterAthlete,multiSportManaged=false}:{accountRole:AccountRole;juniorMode?:boolean;sport:Sport;setSport:React.Dispatch<React.SetStateAction<Sport>>;goals:Goal[];workouts:Workout[];results:Result[];profile:Profile;setProfile:React.Dispatch<React.SetStateAction<Profile>>;onProfileSaved?:(profile?:Profile)=>void;readiness:ReadinessLog[];competitions:CompetitionLog[];dev:DevelopmentItem[];program:TrainingProgram|null;weeklyReviews:WeeklyReview[];setWeeklyReviews:React.Dispatch<React.SetStateAction<WeeklyReview[]>>;coachWeeklyReviews:CoachWeeklyReview[];developmentSystem:DevelopmentSystemState;testTargets:TestTarget[];workspaceRole:WorkspaceRole;onboardingDismissed:boolean;setOnboardingDismissed:React.Dispatch<React.SetStateAction<boolean>>;setTab:React.Dispatch<React.SetStateAction<Tab>>;onNavigate:NavigateTo;editProfileRequest:number;openCoachTeams?:()=>void;coachSelectedAthleteName?:string;loginSessionKey?:string;coachCloudRoster?:CoachCloudAthleteState[];coachRosterCloudStatus?:"idle"|"loading"|"ready"|"error";selectCoachRosterAthlete?:((workspaceId:string)=>void);multiSportManaged?:boolean}){
 ...
 const choosePlayerPhoto=async(file?:File)=>{
  if(!canEditProfile||!file)return;
  if(!file.type.startsWith("image/")){setPhotoMessage("Choose a photo from your camera or photo library.");return}
  if(file.size>12*1024*1024){setPhotoMessage("Choose a photo smaller than 12 MB.");return}
  setPhotoBusy(true);setPhotoMessage("Preparing photo…");
  try{
   const photoUrl=await resizePlayerPhoto(file);
 ...
 const ratedHomeCompetitions=competitions.filter(c=>c.sport===sport&&c.rating>0);
 const competitionAvg=ratedHomeCompetitions.length?Math.round(ratedHomeCompetitions.reduce((a,c)=>a+c.rating,0)/ratedHomeCompetitions.length*10):0;
 ...
 const activeGoals=goals.filter(g=>(g.status||"Active")!=="Complete");
 const latestCoachHomeReview=coachWeeklyReviews.slice().sort((a,b)=>b.weekStart.localeCompare(a.weekStart))[0];
 const sharedDevelopmentFocus=openDev[0]?.title||prioritySkill||activeGoals[0]?.title||"Build one quality development habit.";
 ...
 const ratedRecentComps=recentComps.filter(c=>c.rating>0);
 const avgRecentComp=ratedRecentComps.length?Math.round(ratedRecentComps.reduce((a,c)=>a+c.rating,0)/ratedRecentComps.length*10)/10:0;
 ...
 useEffect(()=>{setRoleSetupTemporarilyHidden(false)},[setupPct,accountRole]);
 useEffect(()=>{
 ...
 const goToSetupItem=(step:HomeSetupStep)=>{
  setRoleSetupTemporarilyHidden(true);
  if(step.action){step.action();return}
  if(!step.tab)return;
  onNavigate(step.tab,step.target?`setup-${step.target}`:undefined);
...
const roleActions:{title:string;detail:string;tab:Tab;target?:string}[]=workspaceRole==="Coach"
  ?[
    {title:"Review readiness",detail:`${avgReadiness||"No"} readiness score`,tab:"Coach",target:"setup-readiness"},
  ...
  :workspaceRole==="Parent"
  ?[
    {title:"Review schedule",detail:`${upcoming.length+sportComps.length} upcoming items`,tab:"Calendar",target:upcoming.length?"workout-log":"setup-calendar"},
    {title:"Review progress",detail:nearGoals?`${nearGoals} goal${nearGoals===1?"":"s"} near completion`:"See current progress",tab:"Analytics"},
    {title:"Support recovery",detail:avgReadiness?`${avgReadiness}/100 readiness`:"Review recovery and notes",tab:"Coach",target:"parent-recovery-summary"}
   ]
  :[
    {title:"Today's focus",detail:nextAction,tab:openDev[0]?"Development":"Calendar",target:openDev[0]?undefined:upcoming.length?"workout-log":"setup-calendar"},
    {title:"Training status",detail:avgReadiness>=75?"Ready for quality work":"Monitor recovery",tab:"Coach",target:"setup-readiness"},
...
const requestProfileUpdate=()=>{
  onNavigate("Coach","shared-support-notes");
};
...
const signals:PerformanceSignal[]=[
  ...
  return <><div className="activeAthleteBanner"><small>COACH WORKSPACE</small><b>{coachSelectedAthleteName||"No Player selected"}</b><span>{coachSelectedAthleteName?`${sport}${profile.position?` · ${profile.position}`:""}`:"Select a Player from Teams or the Command Center"}</span></div>
   ...
   {coachSelectedAthleteName&&<div className="card sharedDevelopmentFocusCard coachFocusCard">
    ...
    <div className="sharedFocusGrid">
     ...
     <div><small>NEXT COACH ACTION</small><b>{avgReadiness>0&&avgReadiness<60?"Review readiness before adding load":"Confirm the next development action"}</b><span>Use evidence and Player perspective — not a practice-plan generator.</span></div>
    </div>
   </div>}
 ...
 {accountRole==="Player"?<div className={"hero homeHero playerSimpleHero "+(juniorMode?"juniorHomeHero":"")}><div className="heroCopy"><small>{juniorMode?"JUNIOR PLAYER · TODAY":"PLAYER · TODAY"}</small><h1>{juniorMode?`Hi, ${profile.name||"Athlete"}!`:`${playerGreeting}, ${profile.name||"Athlete"}`}</h1><p>{juniorMode?"Ready for one small step today?":`${sport}${profile.position?" · "+profile.position:""}${profile.team?" · "+profile.team:""}`}</p></div><button type="button" className={"playerHelpCue "+(juniorMode?"featureAction juniorProgressButton":"")} onClick={()=>{setTab("Analytics");window.setTimeout(()=>window.scrollTo({top:0,behavior:"smooth"}),40)}}>{juniorMode?"How am I doing? →":"See My Progress →"}</button></div>:<div className="hero homeHero phase30Hero"><div className="heroCopy"><small>ATHLETE OVERVIEW</small><h1>{profile.name}</h1><p>{sport}{profile.position?" · "+profile.position:""}{profile.team?" · "+profile.team:""} · {profile.season}</p><div className="heroBadges"><span>{score}/100 Performance</span><span>{avgReadiness||"—"} Readiness</span><span>{streak} Day Streak</span></div></div><div className="heroScoreOrb"><strong>{score}</strong><small>PERFORMANCE</small></div></div>}

 {accountRole==="Player"&&<div className={"card sharedDevelopmentFocusCard playerFocusCard "+(juniorMode?"juniorSharedFocus":"")}>
  <div className="sectionHead"><div><small>{juniorMode?"MY FOCUS":"SHARED DEVELOPMENT FOCUS"}</small><h2>{juniorMode?"What am I working on?":"What am I working on right now?"}</h2></div><button onClick={()=>setTab(activeGoals[0]?"Goals":"Development")}>{juniorMode?(activeGoals[0]?"My Goal":"My Skills"):"Open"}</button></div>
  <div className="sharedFocusGrid">
   <div><small>{juniorMode?"MY GOAL":"PLAYER GOAL"}</small><b>{activeGoals[0]?.title||sharedDevelopmentFocus}</b><span>{activeGoals[0]?`${activeGoals[0].progress}% complete`:"One clear focus is enough."}</span></div>
   {!juniorMode&&<div><small>COACH FOCUS</small><b>{latestCoachHomeReview?.nextWeekFocus||openDev[0]?.title||"No Coach focus shared yet"}</b><span>{latestCoachHomeReview?.coachMessage||"Your Coach can add development direction without changing your goal."}</span></div>}
   <div><small>{juniorMode?"NEXT":"NEXT STEP"}</small><b>{nextAction}</b><span>{juniorMode?"One small step at a time.":"Use Today to connect this focus to your next action."}</span></div>
  </div>
 </div>}
 {accountRole==="Player"&&<section className="playerSimpleDashboard">
  <div className="playerPrimaryTask">
   <div><small>{juniorMode?"MY NEXT STEP":!todayReadiness?"START HERE":"NEXT"}</small><h2>{juniorMode?(playerPrimaryAction.title==="Complete Daily Check-In"?"How do I feel today?":playerPrimaryAction.title==="Complete Weekly Review"?"Tell me about my week":playerPrimaryAction.title):playerPrimaryAction.title}</h2><p>{juniorMode?"Tap the big button when you're ready.":playerPrimaryAction.detail}</p></div>
   <button className="featureAction" onClick={()=>onNavigate(playerPrimaryAction.tab,playerPrimaryAction.title==="Complete Daily Check-In"?"setup-readiness":playerPrimaryAction.title==="Complete Weekly Review"?"setup-weekly-review":playerPrimaryAction.tab==="Calendar"?"workout-log":undefined)}>{juniorMode?"Let's Go":!todayReadiness?"Start":"Open"} →</button>
  </div>
  <div className="playerSimpleGrid">
   <button onClick={()=>onNavigate("Coach","setup-readiness")}><small>{juniorMode?"HOW DO I FEEL?":"HOW I FEEL TODAY"}</small><b>{todayReadiness?`${todayReadinessScore} · ${readinessStatus(todayReadinessScore)}`:"Check in"}</b><span>{todayReadiness?"Tap for the details":juniorMode?"Tell us with a few easy choices":"Sleep · energy · soreness · stress"}</span></button>
   <button onClick={()=>onNavigate("Calendar",upcoming[0]?"workout-log":"setup-calendar")}><small>NEXT TRAINING</small><b>{upcoming[0]?.name||"Nothing scheduled"}</b><span>{upcoming[0]?`${friendlyDate(upcoming[0].date)} · ${upcoming[0].minutes} min`:"Open Schedule"}</span></button>
  ...
  {juniorMode?<div className="juniorWinsStrip">
    <button onClick={()=>setTab("Goals")}><span>★</span><b>My Goal</b><small>What I want to improve</small></button>
    <button onClick={()=>setTab("Analytics")}><span>↗</span><b>My Wins</b><small>See how I'm doing</small></button>
    <button onClick={()=>setTab("Development")}><span>◆</span><b>My Skills</b><small>What I'm learning next</small></button>
   </div>:<><div className="playerWeekStrip">
 ...
 <div className="roleActionGrid playerOptionalHome">{roleActions.map(x=><button key={x.title} onClick={()=>onNavigate(x.tab,x.target)}><small>{x.title}</small><b>{x.detail}</b><span>Open →</span></button>)}</div>
 ...
 <div className="card playerProfileCard setupAnchor" id="setup-profile" tabIndex={-1}>
  ...
  <div className="commercialProfilePhotoPanel">
   ...
   <input ref={photoInputRef} className="commercialPhotoInput" type="file" accept="image/*" onChange={e=>{void choosePlayerPhoto(e.target.files?.[0]);e.currentTarget.value=""}}/>
   {photoMessage&&<span className="commercialPhotoMessage" role="status">{photoMessage}</span>}
  </div>
 ...
 <details className="simpleDisclosure advancedTools homeAdvancedDetail"><summary><div><b>Performance Summary</b><small>Readiness, goals, testing, and competition</small></div><span>Open</span></summary><div className="simpleDisclosureBody"><div className="sectionDivider"><span><i/>Performance Summary</span></div>
...
function Goals({viewRole,actualRole,authorName,goals,setGoals,juniorMode=false}:{viewRole:AccountRole;actualRole:AccountRole;authorName:string;goals:Goal[];setGoals:React.Dispatch<React.SetStateAction<Goal[]>>;juniorMode?:boolean}){
 const juniorGoalOwner=juniorMode&&viewRole==="Player";
 const canOwnGoals=juniorGoalOwner||((viewRole===actualRole)&&canRole(actualRole,"createPlayerGoal")&&canRole(actualRole,"updatePlayerGoal"));
 ...
 const add=(forcedType?:Goal["type"])=>{
  if(!canOwnGoals||!title.trim())return;
  setGoals(g=>[{id:Date.now(),title:title.trim(),progress:0,type:forcedType||type,category,deadline,target:target.trim(),status:"Active",notes:notes.trim(),coachFeedback:[]},...g]);
 ...
 {juniorMode&&viewRole==="Player"&&<div className="card setupAnchor createGoalCard juniorGoalCard juniorGoalEntryCard" id="setup-goals" tabIndex={-1}>
  ...
  <div className="juniorGoalBuilder juniorGoalInputArea">
   <label>My goal<input autoFocus value={title} onChange={e=>setTitle(e.target.value)} placeholder="Type your goal here..." aria-label="My goal"/></label>
   <label>What kind of goal?<select value={category} onChange={e=>setCategory(e.target.value)}><option>Skill</option><option>Speed</option><option>Strength</option><option>Training</option><option>Competition</option><option>Recovery</option><option>Personal</option></select></label>
   <label>How will you know you did it?<input value={target} onChange={e=>setTarget(e.target.value)} placeholder="e.g. I can do it 5 times in a row"/></label>
   <label>Anything you want to remember?<input value={notes} onChange={e=>setNotes(e.target.value)} placeholder="Optional"/></label>
   <div className="juniorGoalHelp"><span>★</span><p>A Parent can help you type. This is still <b>your</b> goal.</p></div>
   <button type="button" className="featureAction primary juniorBigAction juniorSaveGoalButton" disabled={!title.trim()} onClick={()=>add("Short-term")}>Save My Goal</button>
  </div>
 </div>}
 ...
 {canOwnGoals&&!juniorMode?<div className="card setupAnchor createGoalCard" id="setup-goals" tabIndex={-1}>
  ...
  <button type="button" className="primary" onClick={()=>add()}>{viewRole==="Admin"?"Create Goal · Admin Override":"Create My Goal"}</button>
 </div>:!juniorMode?<div className="card goalReadOnlyOwnership"><span className="tag">NO GOAL CREATION</span><h2>{viewRole==="Coach"?"Wait for the Player to create the goal":viewRole==="Parent"?"Player creates the goal":"Player ownership protected"}</h2><p>{viewRole==="Coach"?"Once a Player goal exists, you can add a Suggestion or Comment directly on that goal.":viewRole==="Parent"?"You can review goals and support the athlete, but the Player creates and updates them.":"This view does not create or alter Player goals."}</p></div>:null}
...
function Calendar({accountRole,sport,workouts,setWorkouts,profile,seasonEvents,setSeasonEvents,trainingBlocks,setTrainingBlocks,competitions}:{accountRole:AccountRole;sport:Sport;workouts:Workout[];setWorkouts:any;profile:Profile;seasonEvents:SeasonEvent[];setSeasonEvents:React.Dispatch<React.SetStateAction<SeasonEvent[]>>;trainingBlocks:TrainingBlock[];setTrainingBlocks:React.Dispatch<React.SetStateAction<TrainingBlock[]>>;competitions:CompetitionLog[]}){
 ...
 const addWorkout=()=>{
   ...
   const item:Workout={id:Date.now(),date,name,category:cat,minutes:+minutes,completed:false,sport,intensity,focus:focusNote.trim(),source:"Manual",assignedByRole:accountRole};
 ...
 <div className="card setupAnchor" id="workout-log" tabIndex={-1}><h2>Workout Log</h2>{rows.length===0?<p>No workouts yet.</p>:rows.map(w=><WorkoutLogCard key={w.id} workout={w} onComplete={completeWorkout} onReopen={reopenWorkout}/>)}</div>
...
function Analytics({sport,profile,results,goals,workouts,readiness,competitions,accountRole="Parent",actualAccountRole=accountRole,setTab,setDev,setGoals,setWorkouts}:{sport:Sport;profile:Profile;results:Result[];goals:Goal[];workouts:Workout[];readiness:ReadinessLog[];competitions:CompetitionLog[];accountRole?:AccountRole;actualAccountRole?:AccountRole;setTab?:React.Dispatch<React.SetStateAction<Tab>>;setDev?:React.Dispatch<React.SetStateAction<DevelopmentItem[]>>;setGoals?:React.Dispatch<React.SetStateAction<Goal[]>>;setWorkouts?:React.Dispatch<React.SetStateAction<Workout[]>>}){
 ...
 const sportComps=competitions.filter(c=>c.sport===sport);
 const ratedSportComps=sportComps.filter(c=>c.rating>0);
 const competitionScore=ratedSportComps.length?Math.round(ratedSportComps.reduce((a,c)=>a+c.rating,0)/ratedSportComps.length*10):0;
 ...
 <section className="developmentIntelligenceCard">
  <div className="developmentIntelHeader"><div><small>DEVELOPMENT NEXT STEP</small><h2>Review → Decide → Act</h2><p>Review the current results, why they matter, and the next step in the development plan.</p></div><span className="tag">{prioritySignal.category}</span></div>
  ...
  </div>:<div className="developmentIntelReadOnly"><b>Read-only support view</b><span>{accountRole==="Parent"?"Parents can review the same information and support the Player without changing Player-owned goals or Coach-managed development direction.":"This role sees the same information and next step without creating a Player goal."}</span></div>}
...
function Program({accountRole,sport,profile,dev,results,readiness,program,setProgram,setWorkouts}:{accountRole:AccountRole;sport:Sport;profile:Profile;dev:DevelopmentItem[];results:Result[];readiness:ReadinessLog[];program:TrainingProgram|null;setProgram:React.Dispatch<React.SetStateAction<TrainingProgram|null>>;setWorkouts:any}){
 ...
 <div className="hero scienceProgramHero"><small>EVIDENCE-INFORMED · AGE-AWARE</small><h1>Sport + Position + Age Workout Builder</h1><p>{sport}{profile.position?" · "+profile.position:" · Select a position in Player Profile"}{profile.age?` · age ${profile.age}`:" · age required"} · The plan uses sport demands, exact position, chronological age, season phase, readiness, and available equipment.{sport==="Ice Hockey"?" Hockey training in this builder is OFF-ICE / dryland only.":""}</p></div>
...
function Readiness({sport,profile,readiness,setReadiness,weeklyReviews,coachNotes,setCoachNotes,program,workouts,accountRole="Coach",authorName="",saveSharedNotes,juniorMode=false,onOpenTrackers,trackerConnectedCount=0}:{sport:Sport;profile:Profile;readiness:ReadinessLog[];setReadiness:React.Dispatch<React.SetStateAction<ReadinessLog[]>>;weeklyReviews:WeeklyReview[];coachNotes:CoachNote[];setCoachNotes:React.Dispatch<React.SetStateAction<CoachNote[]>>;program:TrainingProgram|null;workouts:Workout[];accountRole?:AccountRole;authorName?:string;saveSharedNotes?:((notes:unknown[])=>Promise<void>);juniorMode?:boolean;onOpenTrackers?:()=>void;trackerConnectedCount?:number}){
 ...
 return <><div className="sectionDivider"><span><i/>{juniorMode?"How I Feel":"Readiness"}</span></div><div className={"hero phase34Hero "+(juniorMode?"juniorReadinessHero":"")}><small>{juniorMode?"JUNIOR PLAYER":"RECOVERY & READINESS"}</small><h1>{juniorMode?"How do I feel today?":"Daily Readiness"}</h1><p>{juniorMode?"There are no wrong answers. Pick what feels closest.":`${sport} · Recovery check-ins, trends, and training guidance.`}</p></div>
 ...
 {(accountRole==="Player"||accountRole==="Parent")&&onOpenTrackers&&<button type="button" className="recoveryTrackerCta" onClick={onOpenTrackers}>
  <span className="trackerDiscoveryIcon"><PremiumAppIcon name="recovery"/></span>
  <span><small>CONNECTED RECOVERY DATA</small><b>{trackerConnectedCount?"Manage workout & sleep trackers":"Connect workout & sleep trackers"}</b><em>{trackerConnectedCount?`${trackerConnectedCount} source${trackerConnectedCount===1?"":"s"} connected · sync sleep, HRV and workouts`:"Bring sleep and workout data into Recovery & Readiness."}</em></span>
  <strong>{trackerConnectedCount?"MANAGE →":"CONNECT →"}</strong>
 </button>}
 ...
 {canRole(accountRole,"playerDailyCheckIn")?<div className={"card setupAnchor "+(juniorMode?"juniorCheckinCard":"")} id="setup-readiness" tabIndex={-1}><div className="sectionHead"><div><span className="tag">{accountRole==="Admin"?"ADMIN OVERRIDE":juniorMode?"MY CHECK-IN":"PLAYER ONLY"}</span><h2>{accountRole==="Admin"?"Player Daily Check-In":juniorMode?"Tell us how you feel":"My Daily Check-In"}</h2><small>{accountRole==="Admin"?"Admin can create or correct Player readiness data when needed.":juniorMode?"A Parent can help, but choose the answers that feel true for you.":"Your answers are shared with linked Parents and Coaches after you save."}</small></div></div>{juniorMode?<div className="juniorCheckin">
   <label>How many hours did you sleep?<select value={sleep} onChange={e=>setSleep(e.target.value)}>{["4","5","6","7","8","9","10","11","12"].map(x=><option key={x}>{x} hours</option>)}</select></label>
   <div className="juniorFeelingQuestion"><b>How much energy do you have?</b><div>{[["3","😴","Tired"],["5","😐","Okay"],["7","🙂","Good"],["9","😄","Great"]].map(([v,icon,label])=><button type="button" className={energy===v?"active":""} key={v} onClick={()=>setEnergy(v)}><span>{icon}</span><b>{label}</b></button>)}</div></div>
   <div className="juniorFeelingQuestion"><b>How does your body feel?</b><div>{[["2","💚","Feels good"],["4","🙂","A little sore"],["6","😕","Sore"],["8","🛑","Very sore"]].map(([v,icon,label])=><button type="button" className={soreness===v?"active":""} key={v} onClick={()=>setSoreness(v)}><span>{icon}</span><b>{label}</b></button>)}</div></div>
   <div className="juniorFeelingQuestion"><b>How does your mind feel?</b><div>{[["2","😌","Calm"],["4","🙂","Okay"],["6","😟","Worried"],["8","😣","Stressed"]].map(([v,icon,label])=><button type="button" className={stress===v?"active":""} key={v} onClick={()=>setStress(v)}><span>{icon}</span><b>{label}</b></button>)}</div></div>
   <label>Anything you want to tell us?<input value={notes} onChange={e=>setNotes(e.target.value)} placeholder="Optional"/></label>
   <button className="primary juniorBigAction" onClick={saveReadiness}>Save How I Feel</button>
...
function Competition({accountRole,sport,competitions,setCompetitions,profile}:{accountRole:AccountRole;sport:Sport;competitions:CompetitionLog[];setCompetitions:React.Dispatch<React.SetStateAction<CompetitionLog[]>>;profile:Profile}){
 ...
 const save=()=>{
  ...
  const item:CompetitionLog={
   ...
   enteredByRole:accountRole,entryKind:"Full"
  };
  setCompetitions(x=>{
 ...
 const ratedMine=mine.filter(x=>x.rating>0);
 const confidentMine=mine.filter(x=>(x.confidence||0)>0);
 const games=mine.length;
 const avgRating=ratedMine.length?Math.round(ratedMine.reduce((a,x)=>a+x.rating,0)/ratedMine.length*10)/10:0;
 const avgConfidence=confidentMine.length?Math.round(confidentMine.reduce((a,x)=>a+(x.confidence||0),0)/confidentMine.length*10)/10:0;
 ...
 const recentRatings=ratedMine.slice(0,8).reverse();
 const bestRating=ratedMine.length?Math.max(...ratedMine.map(x=>x.rating)):0;
 const lastThree=ratedMine.slice(0,3);
 ...
 const ratedLastFive=lastFive.filter(c=>c.rating>0);
 const confidentLastFive=lastFive.filter(c=>(c.confidence||0)>0);
 const formAvg=ratedLastFive.length?Math.round(ratedLastFive.reduce((a,c)=>a+c.rating,0)/ratedLastFive.length*10)/10:0;
 const confidenceAvg=confidentLastFive.length?Math.round(confidentLastFive.reduce((a,c)=>a+(c.confidence||0),0)/confidentLastFive.length*10)/10:0;
 const recentWins=lastFive.filter(c=>(c.result||"").toLowerCase().startsWith("w")).length;
 const bestGame=ratedLastFive.slice().sort((a,b)=>b.rating-a.rating)[0];
return <><div className="hero"><small>COMPETITION</small><h1>Competition</h1><p>{profile.name} · {sport}{profile.position?" · "+profile.position:""} · Track game performance, confidence, and post-event learning.</p></div>
 ...
 {mine.length===0?<div className="card"><p>No competitions logged yet.</p></div>:mine.map(g=><div className="card competitionCard" key={g.id}>
  <div className="row"><div><span className="tag">{g.eventType}</span><h2>{g.opponent}</h2><p>{g.date}{g.result?" · "+g.result:""}{g.location?" · "+g.location:""}{g.minutes?" · "+g.minutes+" min":""}</p></div><div className="ratingBadge">{g.rating>0?<>{g.rating}<small>/10</small></>:<><span>RESULT</span><small>logged</small></>}</div></div>
...
function Reports({sport,profile,goals,workouts,results,dev,program,readiness,competitions,reportNotes,setReportNotes}:{sport:Sport;profile:Profile;goals:Goal[];workouts:Workout[];results:Result[];dev:DevelopmentItem[];program:TrainingProgram|null;readiness:ReadinessLog[];competitions:CompetitionLog[];reportNotes:ReportNote[];setReportNotes:React.Dispatch<React.SetStateAction<ReportNote[]>>}){
 ...
 const ratedReportCompetitions=sportCompetitions.filter(c=>c.rating>0);
 const avgRating=ratedReportCompetitions.length?Math.round(ratedReportCompetitions.reduce((a,c)=>a+c.rating,0)/ratedReportCompetitions.length*10)/10:0;
 const recentReadiness=readiness.slice(0,7);
 const avgReadiness=recentReadiness.length?Math.round(recentReadiness.reduce((a,r)=>a+readinessScoreV2(r,Number(profile.age||0)),0)/recentReadiness.length):0;
 ...
 <div className="reportScore"><div><small>OVERALL PROGRESS SCORE</small><strong>{overall}</strong><span>/100</span></div><div><b>{overall>=80?"Strong Progress":overall>=60?"Building Momentum":"Needs Attention"}</b><p>Combined from goals, training, readiness, competition, and testing activity.</p></div></div>
 ...
 <div className="card"><div className="sectionHead"><h2>Testing Progress</h2><div className="reportActions"><button onClick={exportSummary}>Export Summary CSV</button><button onClick={copySummary}>Copy Summary</button><button onClick={downloadSnapshot}>Download Snapshot</button><button onClick={()=>window.print()}>Print Report</button></div></div>
  {grouped.length===0?<p>Add repeated test results to see measurable progress.</p>:grouped.slice(0,6).map(g=><div className="insightRow" key={g.name}><div><b>{g.name}</b><small>{g.count} results · Baseline {g.first} {g.unit} → Current {g.last} {g.unit}</small></div><strong className={g.imp>=0?"good":"bad"}>{g.count>1?(g.imp>=0?"+":"")+g.imp+"%":"New"}</strong></div>)}
 </div>
...
function Roster({accountRole,sport,profile,roster,setRoster,activeAthleteId,switchAthlete,setTab,setEditProfileRequest,goals,currentWorkouts,currentResults,currentDev,currentReadiness,currentCompetitions,currentDevelopmentSystem,currentTestTargets,currentCoachWeeklyReviews,cloudSelectedAthleteName,coachCloudRoster,coachRosterCloudStatus,selectCoachRosterAthlete,openCoachTeams,openCoachInvitePlayer,createAdminTestAthlete}:{accountRole:AccountRole;sport:Sport;profile:Profile;roster:AthleteRecord[];setRoster:React.Dispatch<React.SetStateAction<AthleteRecord[]>>;activeAthleteId:string;switchAthlete:(a:AthleteRecord)=>void;setTab:React.Dispatch<React.SetStateAction<Tab>>;setEditProfileRequest:React.Dispatch<React.SetStateAction<number>>;goals:Goal[];currentWorkouts:Workout[];currentResults:Result[];currentDev:DevelopmentItem[];currentReadiness:ReadinessLog[];currentCompetitions:CompetitionLog[];currentDevelopmentSystem:DevelopmentSystemState;currentTestTargets:TestTarget[];currentCoachWeeklyReviews:CoachWeeklyReview[];cloudSelectedAthleteName?:string;coachCloudRoster:CoachCloudAthleteState[];coachRosterCloudStatus:"idle"|"loading"|"ready"|"error";selectCoachRosterAthlete?:((workspaceId:string)=>void);openCoachTeams?:()=>void;openCoachInvitePlayer?:()=>void;createAdminTestAthlete?:((input:{name:string;sport:string;age:number;position:string;team:string})=>Promise<{athleteId:string;workspaceId:string;name:string}>);}){
 ...
 const add=async()=>{
  if(!canManageProfiles||!name.trim()||creatingPlayer)return;
  setAddPlayerMessage("");
  const ageNumber=Number(age);
  if(!Number.isFinite(ageNumber)||ageNumber<6||ageNumber>99){
   setAddPlayerMessage("Enter a valid Player age from 6 to 99.");
   return;
  }

  if(createAdminTestAthlete){
   setCreatingPlayer(true);
   try{
    await createAdminTestAthlete({name:name.trim(),sport:newSport,age:ageNumber,position,team});
    setName("");setAge("");setPosition("");setTeam("");setHeight("");setWeight("");
    setAddPlayerMessage("Cloud test Player created. It will remain available after refresh, a new Codespace, or another device.");
   }catch(error:any){
    setAddPlayerMessage(error?.message||"Could not create the cloud test Player.");
   }finally{
  ...
  setRoster(x=>[...x,item]);
  setName("");setAge("");setPosition("");setTeam("");setHeight("");setWeight("");
  setAddPlayerMessage("Local Player profile created on this device.");
 };
 ...
 type CoachRosterAnalytics={
  id:string;name:string;sport:Sport;position:string;team:string;photoUrl?:string;
 ...
 const coachAnalyticsFor=(a:AthleteRecord):CoachRosterAnalytics=>{
  ...
  return {
   id:a.id,name:a.name,sport:athleteSport,position:a.position,team:a.team,photoUrl:a.photoUrl,
 ...
 const summaries:RosterSummary[]=coachRosterAnalytics.map(a=>({
  id:a.id,name:a.name,sport:a.sport,position:a.position,team:a.team,photoUrl:a.photoUrl,
  ...
  <div className="coachRosterAnalyticsGrid">
   {coachRosterSorted.map(a=><article className={"coachPlayerAnalyticsCard "+a.status.toLowerCase()+(isSelectedCoachRecord(a)?" active":"")} key={a.id}>
    <div className="coachPlayerAnalyticsTop">
     <PlayerPhotoAvatar name={a.name} photoUrl={a.photoUrl} size={48} className="rosterAvatar"/>
 ...
 {canManageProfiles&&<><div className="card"><div className="sectionHead"><h2>Roster Overview</h2><span className="tag">{filteredRoster.length} shown</span></div><div className="rosterOverviewGrid">{filteredRoster.map(a=><div className={"rosterSummaryCard "+(activeAthleteId===a.id?"activeRoster":"")} key={a.id}>
  <div className="rosterSummaryTop"><PlayerPhotoAvatar name={a.name} photoUrl={a.photoUrl} size={48} className="rosterAvatar"/><div><b>{a.name}</b><small>{a.sport}{a.position?" · "+a.position:""}{a.team?" · "+a.team:""}</small></div><strong>{a.score}</strong></div>
 ...
 {canManageProfiles&&<><div className="card rosterAddCard setupAnchor" id="roster-add-athlete" tabIndex={-1}><div className="sectionHead"><h2>{createAdminTestAthlete?"Create Cloud Test Player":"Add Player"}</h2><span className="tag">{createAdminTestAthlete?"CLOUD TEST":"New Athlete"}</span></div><p className="rosterAddIntro">{createAdminTestAthlete?"Creates a beta test athlete in Supabase so the Player persists across Codespaces, browsers, and app updates. This does not create a real Player login.":"Create a separate athlete profile with sport-specific position, team, measurements, and handedness."}</p>
  <div className="two">
  ...
  <button className="primary" disabled={creatingPlayer} onClick={()=>void add()}>{creatingPlayer?"Creating…":createAdminTestAthlete?"Create Cloud Test Player":"Create Player Profile"}</button>
  {addPlayerMessage&&<div className="rosterCloudCreateMessage">{addPlayerMessage}</div>}
 </div></>}
 ...
 {canManageProfiles&&<><div className="card rosterManagementCard"><div className="sectionHead rosterManagementHead"><div><h2>{canManageProfiles?"Roster Management":"Coach Roster"}</h2><small>{canManageProfiles?"Admin profile controls":"Select an athlete to review. Profile identity is read-only."}</small></div>{canManageProfiles&&<button className="featureAction rosterManagementAdd" onClick={scrollToAddAthlete}>＋ Add Player</button>}</div>
  <div className="rosterGrid">{all.map(a=><div className={"rosterCard "+(activeAthleteId===a.id?"activeRoster":"")} key={a.id}>
   <PlayerPhotoAvatar name={a.name} photoUrl={a.photoUrl} size={48} className="rosterAvatar"/>
   <div><b>{a.name}</b><small>{a.sport}{a.position?" · "+a.position:""}{a.team?" · "+a.team:""}</small><small>{a.id.startsWith("cloud:")?"Cloud athlete · persists across devices":`${a.height||"—"} · ${a.weight||"—"} · ${a.handedness}`}</small></div>
...
function SmartCoach({sport,profile,goals,workouts,results,dev,program,readiness,competitions}:{sport:Sport;profile:Profile;goals:Goal[];workouts:Workout[];results:Result[];dev:DevelopmentItem[];program:TrainingProgram|null;readiness:ReadinessLog[];competitions:CompetitionLog[]}){
 ...
 const ratedSc=sc.filter(c=>c.rating>0);
 if(ratedSc.length>=3){const rating=Math.round(ratedSc.reduce((a,c)=>a+c.rating,0)/ratedSc.length*10)/10;if(rating<6.5)recs.push({id:"comp",title:"Review Competition Performance",reason:`Average competition rating is ${rating}/10.`,action:"Review the last 3 competition notes and turn one repeat issue into a development objective.",priority:"Medium",category:"Competition"})}
 ...
 return <><div className="sectionDivider"><span><i/>Coach Development Review</span></div><div className="hero"><small>PLAYER DEVELOPMENT SUPPORT</small><h1>Coach Development Review</h1><p>{profile.name} · {sport}{profile.position?" · "+profile.position:""} · Use athlete data to understand development needs. This view does not build practices.</p></div>

 <div className="coachHero"><div><small>DEVELOPMENT CONTEXT</small><strong>{score}</strong><span>/100</span></div><div><b>{score>=80?"Strong Current Results":score>=60?"Useful Development Picture":"More Information Needed"}</b><p>This Coach summary uses goals, training consistency, readiness, and testing activity. It is not a practice-plan score and is separate from the shared Progress score.</p></div></div>
 ...
 <div className="card coachDevelopmentFocus"><div className="sectionHead"><div><small>CURRENT ATHLETE DIRECTION</small><h2>{topPriority}</h2></div><span className={"tag "+risk.toLowerCase()}>{risk} RECOVERY ATTENTION</span></div>
  <p>Use these results to support the athlete's Development Plan, conversations, observations, and reassessment. Practice design stays with the Coach's preferred planning tools.</p>
 </div>
 ...
 <div className="card"><h2>Suggested Development Steps</h2><div className="recommendationList">{recs.map(r=><div className={"recommendation "+r.priority.toLowerCase()} key={r.id}><div className="recPriority">{r.priority}</div><div><span className="tag">{r.category}</span><h2>{r.title}</h2><p><b>Why:</b> {r.reason}</p><p><b>Development step:</b> {r.action}</p></div></div>)}</div></div>
