...
import {trackerCatalog} from "../lib/trackerCatalog";
...
const fallbackTrackerProviders:TrackerProviderStatus[]=trackerCatalog.map(provider=>({...provider,configured:false}));
...
function PremiumHomeOverview({
 ...
 const trackerDiscoveryCta=(accountRole==="Player"||accountRole==="Parent")&&onOpenTrackers?<button type="button" className="trackerDiscoveryCta" onClick={onOpenTrackers} aria-label={trackerConnected.length?"Manage connected performance trackers":"Connect a workout or sleep tracker"}>
  <span className="trackerDiscoveryIcon"><PremiumAppIcon name="recovery"/></span>
  <span className="trackerDiscoveryCopy"><small>WORKOUT + SLEEP DATA</small><b>{trackerConnected.length?"Manage Connected Trackers":"Connect a Performance Tracker"}</b><span>{trackerConnected.length?`${trackerConnected.length} connected source${trackerConnected.length===1?"":"s"} · manage sync and privacy`:"Google Health, Oura, WHOOP and Strava · private to Player + Parent"}</span></span>
  <strong>{trackerConnected.length?"MANAGE →":"CONNECT →"}</strong>
 </button>:null;
...
useEffect(()=>{if(program)localStorage.setItem("trainingProgram",JSON.stringify(program));else localStorage.removeItem("trainingProgram")},[program]);
 ...
 const downloadRecoveryBackup=()=>{
  try{
   const payload={version:"72.3.93",createdAt:new Date().toISOString(),activeAthleteId,snapshot:buildSnapshot()};
 ...
 const trackerDiscoverable=effectiveRole==="Player"||effectiveRole==="Parent";
 ...
 return <div className="app performanceOS" data-text-size={textSize} data-role={effectiveRole} data-junior={juniorPlayerMode?"true":"false"} data-tab={tab} data-sport={sport}><a className="skipLink" href="#main-content">Skip to main content</a>
  ...
  <main>
   ...
   {tab==="Home"&&<PremiumHomeOverview accountRole={effectiveRole} juniorMode={juniorPlayerMode} profile={profile} sport={sport} goals={goals} workouts={workouts} results={results} readiness={readiness} competitions={competitions} dev={dev} setTab={setTab} trackerData={trackerAllowed?trackerData:null} onOpenTrackers={trackerDiscoverable?openTrackerCenter:undefined}/>}
  ...
  {showTrackerSetup&&<ViewportPortal><div className="trackerSetupOverlay" role="dialog" aria-modal="true" aria-label="Connected tracker setup" onClick={()=>setShowTrackerSetup(false)}><div className="trackerSetupScreen" onClick={e=>e.stopPropagation()}>
   ...
   {(accountRole==="Coach"||accountRole==="Admin")?<div className="trackerAccessLocked trackerPreviewLock"><b>PLAYER / PARENT SIGN-IN REQUIRED</b><span>Private tracker authorization cannot be completed from a Coach/Admin preview. Sign in as the actual Player or a linked Parent account to connect a device or service.</span></div>:!trackerAthleteId?<div className="trackerAccessLocked"><b>{accountRole==="Parent"?"CHOOSE A PLAYER":"PLAYER CLOUD RECORD REQUIRED"}</b><span>{accountRole==="Parent"?"Select the Player whose private workout and sleep data you want to connect.":"This Player login is not yet linked to its canonical cloud athlete record. Complete Player setup, then return here."}</span>{accountRole==="Parent"&&betaBridge?.openParentPlayers?<button type="button" className="featureAction trackerChoosePlayer" onClick={()=>{setShowTrackerSetup(false);betaBridge.openParentPlayers?.()}}>Choose Player</button>:<button type="button" className="featureAction trackerChoosePlayer" onClick={()=>{setShowTrackerSetup(false);setGuideStep(0);setShowGuide(true)}}>Open Player Setup</button>}</div>:<>
    ...
    <div className="trackerPrivacyNote"><b>PRIVATE PERFORMANCE DATA</b><span>Only the signed-in Player and Parents linked to this Player can read imported tracker metrics. Coach and Admin roles remain blocked at the database layer.</span></div>
   </>}
  ...
  {showSettings&&<ViewportPortal><div className="settingsOverlay viewportSettingsOverlay" role="dialog" aria-modal="true" aria-label="App settings" onClick={()=>setShowSettings(false)}><div className="settingsCard" onClick={e=>e.stopPropagation()}>
   ...
   {trackerDiscoverable&&<div className="trackerSettings" id="connected-trackers-settings">
    ...
    <button type="button" className="trackerSetupLaunch featureAction" onClick={()=>{setShowSettings(false);setShowTrackerSetup(true)}}><span><PremiumAppIcon name="recovery"/></span><div><small>WORKOUT + SLEEP SERVICES</small><b>Open Tracker Setup</b><em>{trackerData?.connections?.filter(x=>x.status==="connected").length?`${trackerData.connections.filter(x=>x.status==="connected").length} connected source${trackerData.connections.filter(x=>x.status==="connected").length===1?"":"s"} · manage or sync`:`Google Health · Oura · WHOOP · Strava`}</em></div><strong>OPEN →</strong></button>
    <div className="trackerSettingsHint">Player and authorized Parent accounts can connect and view private tracker data. Coach and Admin accounts cannot.</div>
   </div>}
